...
export const adminService = {
  ...
  async approve(type: string, id: string) {
    ...
    if (entityLat && entityLng && type !== "category") {
      try {
        const delta = broadcastKm / 111; // 1 degree ≈ 111 km
        const { data: nearbyUsers } = await sb
          .from("users")
          .select("id")
          .not("lat", "is", null)
          .not("lng", "is", null)
          .gte("lat", entityLat - delta)
          .lte("lat", entityLat + delta)
          .gte("lng", entityLng - delta)
          .lte("lng", entityLng + delta)
          .neq("id", ownerId ?? "")
          .limit(200);

        const userIds = (nearbyUsers ?? []).map((u: any) => u.id as string);
        if (userIds.length > 0) {
