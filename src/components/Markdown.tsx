...
// A small, dependency-free markdown renderer scoped to what the STRYT policy
// documents actually use: headings, horizontal rules, blockquotes, unordered
// lists, tables, paragraphs, and inline bold/italic/code/links. Cross-document
// links written as "(something.md)" are rewritten to the in-app /legal route so
// the policies interlink correctly; mailto:/http(s) links open externally. All
// colours come from design tokens (no hardcoded hex) so the color linter passes.
...
export default function Markdown({ source }: { source: string }) {
  ...
  while (i < lines.length) {
    ...
    if (h) {
      ...
      blocks.push(<Tag key={k} style={H_STYLES[lvl]}>{renderInline(h[2], "h" + k)}</Tag>);
      k++; i++; continue;
    }
    ...
    if (/^\s*[-*]\s+/.test(line)) {
      ...
      blocks.push(
        <ul key={k} style={{ margin: "8px 0", paddingLeft: 22, color: "var(--ink-700)", fontSize: 14, lineHeight: 1.6 }}>
          {items.map((it, ix) => <li key={ix} style={{ marginBottom: 4 }}>{renderInline(it, "li" + k + ix)}</li>)}
        </ul>
      );
      k++; continue;
    }
