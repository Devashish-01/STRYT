...
export default function ShareCard({ title, subtitle, image, meta, url, options, onClose, upiId, paymentQrUrl }: Props) {
  ...
  async function downloadQrImage() {
    ...
    try {
      ...
      a.click();
      showToast("QR Scanner Image Downloaded!");
    } catch {
      showToast("Could not generate QR image download");
    } finally {
            ...
            >
              ...
              {qrMode === "payment" && hasPaymentQr ? (
                <>
                  ...
                  <div className="tiny semi" style={{ marginTop: 10, opacity: 0.85, letterSpacing: 0.5 }}>SCAN TO PAY</div>
                </>
              ) : (
                <>
                  <div style={{ background: "#fff", borderRadius: 16, padding: 12, boxShadow: "var(--shadow)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 12 }}>
            ...
            {channels.map((c) => {
              ...
              return (
                <button key={c.label} className="col center" style={{ gap: 7 }} onClick={c.onClick}>
                  ...
                  <span className="tiny semi" style={{ fontSize: 11 }}>{c.label}</span>
                </button>
              );
            })}
          </div>
          ...
          >
            ...
            <div className="row between no-print" style={{ marginBottom: 16 }}>
              <div className="bold h3 row gap-6">
                <Printer size={20} color="var(--brand-700)" /> Counter Stand Preview
              </div>
              <button className="icon-btn-sm" onClick={() => setShowPrintModal(false)} aria-label="Close preview">
                <X size={18} />
              </button>
            </div>

            {/* Acrylic / Tabletop Counter Stand Poster Card */}
            <div
              style={{
            ...
            >
              {/* Counter Stand Fold Indicator Line (hidden on print) */}
              <div className="tiny semi no-print" style={{ background: "var(--brand-50)", color: "var(--brand-700)", padding: "6px 12px", borderBottom: "1px dashed var(--brand-300)", fontSize: 11, letterSpacing: 0.5, textAlign: "center" }}>
                ✂️ Fold along this line to stand on counter
              </div>
              ...
              <div style={{ background: "linear-gradient(135deg, var(--brand-700), var(--brand-900))", color: "#fff", padding: "18px 20px", textAlign: "center" }}>
                ...
                <div className="tiny" style={{ opacity: 0.85, fontSize: 11, letterSpacing: 0.5, textTransform: "uppercase" }}>
                  Official Local Business Scanner
                </div>
              </div>

              {/* Store Title & Badge Body */}
              <div style={{ padding: "20px 24px 24px", textAlign: "center" }}>
                ...
                {/* Large High-Contrast Scannable QR Code Frame */}
                <div
                  style={{
                    background: "#ffffff",
                    borderRadius: 20,
                ...
                {/* Relatable 3-Step Friendly Scanning Instructions */}
                <div style={{ background: "var(--ink-50)", borderRadius: 16, padding: "14px 16px", marginBottom: 16, textAlign: "left" }}>
                  ...
                  <div className="col gap-6" style={{ fontSize: 12, color: "var(--ink-700)", lineHeight: 1.35 }}>
                    <div className="row gap-8">
                      ...
                      <span>Open your phone's camera or scanner app</span>
                    </div>
                    <div className="row gap-8">
                      <span className="bold" style={{ color: "var(--brand-700)", width: 16 }}>2.</span>
                      <span>Point your camera at this QR code</span>
                    </div>
                    <div className="row gap-8">
                      <span className="bold" style={{ color: "var(--brand-700)", width: 16 }}>3.</span>
                      <span>Tap the link to view our catalog, menu & offers!</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="row center gap-4 tiny muted" style={{ fontSize: 11, fontWeight: 600, color: "var(--ink-500)" }}>
                  ...
                  <span>Support Local Businesses on STRYT</span>
                </div>
              </div>
            </div>
            ...
            <div className="row gap-10 no-print" style={{ marginTop: 20, justifyContent: "flex-end" }}>
              ...
              <button className="btn btn-primary row gap-6" onClick={handlePrint}>
                <Printer size={18} /> Print Counter Stand
              </button>
            </div>
          </div>
        </div>
