...
export default function DeliveryAssignControl({ appointmentId, businessId }: { appointmentId: string; businessId: string }) {
  ...
  const { data: delivery, refetch } = useQuery(() => deliveryService.forAppointment(appointmentId), [appointmentId]);
  const { data: team } = useQuery(() => deliveryService.deliveryTeam(businessId), [businessId]);
