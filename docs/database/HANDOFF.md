...
| ✅ **`users.phone` lockdown — DONE** | Applied 2026-09-20 as `20260994_users_phone_column_lockdown` (ledger `20260920072620`, APPLY_LOG row 57). Its release gate was checked first against the **published** OTA bundle 1.0.68, not the repo. `phone` is now out of `authenticated`'s column grants; a signed-in user reading another user's phone gets `42501`, while `get_own_profile()`, `get_public_profile()` and `provider_leads()` (all SECURITY DEFINER) still serve it with `show_phone_publicly` enforced server-side. |
| **Pending, not applied** | No database changes. `supabase/pending/` is now empty. Live as of 2026-09-20: `20260992`, `20260993`, `20260994` (APPLY_LOG rows 55–57). |
| ⚠ **Ledger hygiene** | The first two 2026-09-20 applies (`20260992`, `20260993`) were run outside the Management API endpoint and wrote **no ledger row**; the agent repaired each by hand (`20260920045700`, `20260920055035`). Applying production DDL through the Supabase SQL Editor leaves the database right and its provenance wrong, and nothing fails at the time — HANDOFF rule 3 exists for exactly this. `20260994` went through `scripts/release/apply-production-migrations.mjs` instead and wrote its own ledger row; that script now takes `--from` / `--to`, so a single migration no longer means editing its constants. Use it, or `apply_migration`. |
