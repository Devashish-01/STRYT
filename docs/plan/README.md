...
| [P07](phases/P07_e2e_test_suite.md) | End-to-end test suite on staging | Agent | 4–6 | 🟨 [Report](reports/P07_REPORT.md) — 21 spec files, 137 tests, 12 min, last run 0 failed/0 flaky. **22 of 52 flows are proven end to end; 25 are proven only to open.** Left: 26 action specs, and 3 consecutive green runs once the branch stops moving |
| [P08](phases/P08_gap_ledger.md) | Gap ledger: verify all 426 findings | Agent | 13 (one per domain) | ✅ Done — 488 rows verified, 0 open at close (430 FIXED, 49 DEFERRED under D2/D17, 9 DECISION). Since then P12/P13/P15 added rows; **7 are open** as of 18 Sept (P12-002, P12-003, P13-001, P13-004, P13-005, P13-006, P15-002) |
...
| [P11](phases/P11_dependencies_bundle_performance.md) | Dependencies, bundle & performance | Agent | 2–3 | 🟨 11.A/11.B/11.D done — no high or critical advisory in shipped code; guest /home 2136 → 1605 KB; RLS initplan and FK indexes applied to staging. Left: Lighthouse (needs a deployed preview, owner step 8). [Report](reports/P11_REPORT.md) |
...
| [P15](phases/P15_store_legal_launch.md) | Store, legal & public launch | Owner + agent | 2–3 (+14-day closed test) | 🟨 15.A done, 15.B drafted ([report](reports/P15_REPORT.md)) — data inventory built from the schema with every citation verified; `play-console/DATA_SAFETY.md` revised (one under-declaration fixed: in-app search history) and made the single source for the form. **Found: `purge-deleted-accounts` has never been deployed, and deletion never removed Aadhaar/PAN documents (P15-001, fixed in code).** 15.C onwards is owner-only |
