...
// (E2E-030) → takes action (the request is cancelled) → the report leaves the queue and the reporter is told it was
// reviewed. Reporting a request made inside the test keeps the shared test salon untouched.
test("admin moderation: report with details, admin takes action, reporter is told", async ({ customer, customer2, admin }) => {
  ...
  await expectAfterReload(customer2, () => customer2.getByText("Your report was reviewed").first());
