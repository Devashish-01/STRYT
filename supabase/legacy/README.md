> **Historical.** This describes the one-time initial bootstrap of the
> Supabase project, already completed — the live database now has real data
> and has diverged from these files in ways this folder doesn't track (see
> `../../REORGANIZATION_PLAN.md` Priority 1). Kept for reference on how the
> project originally got set up, and as a disaster-recovery starting point.
> **Any new schema change goes in `../migrations/` as a new dated file, not
> here.**
