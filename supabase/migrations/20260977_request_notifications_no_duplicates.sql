-- Shops and providers that match a new request got it twice — "New request near you" from the nearby broadcast and
-- "New <category> request" from the targeted insert — both in-app and as push (E2E-024). The broadcast now runs after
-- the targeted inserts and skips anyone they already notified; notifications carry metadata.requestId to tell.
-- Builds on 20260975 (category-group matching): apply after it. Live definition otherwise unchanged.
-- Rollback: supabase/rollbacks/20260977_request_notifications_no_duplicates.rollback.sql (restores the 20260975 body)
