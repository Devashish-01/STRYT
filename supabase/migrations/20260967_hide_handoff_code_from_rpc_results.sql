-- Migration 20260967 — hide the delivery handoff code (OTP) from the rider and staff.
--
-- WHY
--   The handoff code proves the rider met the customer: the customer reads it to
--   the rider, the rider enters it (confirm_handoff), and only then can the
--   delivery be marked DONE. 20260966 removed direct SELECT on
--   appointment_deliveries.handoff_code, but two SECURITY DEFINER functions still
--   returned the whole row, code included:
--     - appointment_update_delivery_status: called by the RIDER on every status
--       change, so the rider received the code and could confirm the handoff
--       without meeting the customer;
--     - assign_delivery: called by business staff (and a rider who also holds the
--       'appointments' scope could assign to themselves).
--   The customer keeps getting the code through my_delivery_progress and the
--   delivery notifications. my_deliveries() already returns null for it.
--
-- WHAT
--   Both functions are the live definitions (pg_get_functiondef, 2026-09-15) with
--   one added line each: the returned row's handoff_code is set to null just
--   before `return v_row`. Everything the function does with the real code
--   (the DONE guard, the customer's ARRIVED notification) runs before that line.
--   Signatures, return types, SECURITY DEFINER and search_path are unchanged, so
--   no app build breaks. Neither app screen reads handoffCode from these calls.
