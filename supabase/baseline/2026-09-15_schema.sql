...
-- @@item Tables 1/91
CREATE TABLE private.admin_action_logs (
...
begin
  ...
  -- The caller is the rider: never hand them the customer's handoff code (20260967).
  v_row.handoff_code := null;
  return v_row;
end $function$
;

-- @@item Functions 42/248
...
begin
  ...
  -- Staff (or a rider holding the appointments scope) must not see the customer's handoff code (20260967).
  v_row.handoff_code := null;
  return v_row;
end $function$
;

-- @@item Functions 43/248
...
-- @@item Functions 45/248
CREATE OR REPLACE FUNCTION public.broadcast_new_listing(p_type text, p_id text)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- @@item Functions 46/248
CREATE OR REPLACE FUNCTION public.broadcast_offer_to_nearby(p_offer_id text, p_radius_km double precision DEFAULT 5)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- @@item Functions 126/248
CREATE OR REPLACE FUNCTION public.grant_team_access(p_business_id text, p_identifier text, p_scopes text[])
 RETURNS TABLE(session_id uuid, grantee_name text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- @@item Functions 190/248
CREATE OR REPLACE FUNCTION public.provider_leads(p_provider_id text)
 RETURNS TABLE(id text, provider_id text, from_user_id text, kind text, note text, handled boolean, created_at timestamp with time zone, from_name text, from_alias text, from_avatar text, from_phone text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
-- @@item Primary keys, unique, check and exclusion constraints 1/192
ALTER TABLE ONLY account_appeals ADD CONSTRAINT account_appeals_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 2/192
...
-- @@item Primary keys, unique, check and exclusion constraints 3/192
...
-- @@item Primary keys, unique, check and exclusion constraints 8/192
...
-- @@item Primary keys, unique, check and exclusion constraints 9/192
...
-- @@item Primary keys, unique, check and exclusion constraints 10/192
...
-- @@item Primary keys, unique, check and exclusion constraints 15/192
...
-- @@item Primary keys, unique, check and exclusion constraints 16/192
ALTER TABLE ONLY appointments ADD CONSTRAINT appointments_party_size_positive CHECK ((party_size >= 1));

-- @@item Primary keys, unique, check and exclusion constraints 17/192
...
-- @@item Primary keys, unique, check and exclusion constraints 18/192
...
-- @@item Primary keys, unique, check and exclusion constraints 19/192
...
-- @@item Primary keys, unique, check and exclusion constraints 20/192
ALTER TABLE ONLY blocked_slots ADD CONSTRAINT blocked_slots_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 21/192
...
-- @@item Primary keys, unique, check and exclusion constraints 22/192
...
-- @@item Primary keys, unique, check and exclusion constraints 23/192
ALTER TABLE ONLY blocked_slots ADD CONSTRAINT blocked_slots_weekday_check CHECK (((weekday >= 0) AND (weekday <= 6)));

-- @@item Primary keys, unique, check and exclusion constraints 24/192
ALTER TABLE ONLY bookmarks ADD CONSTRAINT bookmarks_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 25/192
...
-- @@item Primary keys, unique, check and exclusion constraints 31/192
...
-- @@item Primary keys, unique, check and exclusion constraints 37/192
ALTER TABLE ONLY bulk_deals ADD CONSTRAINT bulk_deals_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 38/192
...
-- @@item Primary keys, unique, check and exclusion constraints 39/192
...
-- @@item Primary keys, unique, check and exclusion constraints 40/192
...
-- @@item Primary keys, unique, check and exclusion constraints 41/192
...
-- @@item Primary keys, unique, check and exclusion constraints 46/192
...
-- @@item Primary keys, unique, check and exclusion constraints 54/192
ALTER TABLE ONLY business_view_logs ADD CONSTRAINT business_view_logs_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 55/192
ALTER TABLE ONLY businesses ADD CONSTRAINT businesses_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 56/192
ALTER TABLE ONLY businesses ADD CONSTRAINT businesses_slug_key UNIQUE (slug);

-- @@item Primary keys, unique, check and exclusion constraints 57/192
...
-- @@item Primary keys, unique, check and exclusion constraints 58/192
...
-- @@item Primary keys, unique, check and exclusion constraints 59/192
...
-- @@item Primary keys, unique, check and exclusion constraints 60/192
...
-- @@item Primary keys, unique, check and exclusion constraints 61/192
...
-- @@item Primary keys, unique, check and exclusion constraints 62/192
ALTER TABLE ONLY catalog_items ADD CONSTRAINT catalog_items_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 63/192
...
-- @@item Primary keys, unique, check and exclusion constraints 64/192
ALTER TABLE ONLY catalog_items ADD CONSTRAINT catalog_items_max_party_positive CHECK ((max_party_size >= 1));

-- @@item Primary keys, unique, check and exclusion constraints 65/192
...
-- @@item Primary keys, unique, check and exclusion constraints 66/192
ALTER TABLE ONLY categories ADD CONSTRAINT categories_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 67/192
...
-- @@item Primary keys, unique, check and exclusion constraints 68/192
ALTER TABLE ONLY client_errors ADD CONSTRAINT client_errors_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 69/192
ALTER TABLE ONLY comment_reactions ADD CONSTRAINT comment_reactions_pkey PRIMARY KEY (comment_id, user_id);

-- @@item Primary keys, unique, check and exclusion constraints 70/192
...
-- @@item Primary keys, unique, check and exclusion constraints 71/192
ALTER TABLE ONLY community_posts ADD CONSTRAINT community_posts_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 72/192
...
-- @@item Primary keys, unique, check and exclusion constraints 73/192
...
-- @@item Primary keys, unique, check and exclusion constraints 74/192
...
-- @@item Primary keys, unique, check and exclusion constraints 75/192
...
-- @@item Primary keys, unique, check and exclusion constraints 81/192
...
-- @@item Primary keys, unique, check and exclusion constraints 82/192
ALTER TABLE ONLY delivery_agent_duty ADD CONSTRAINT delivery_agent_duty_pkey PRIMARY KEY (user_id);

-- @@item Primary keys, unique, check and exclusion constraints 83/192
ALTER TABLE ONLY delivery_batches ADD CONSTRAINT delivery_batches_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 84/192
...
-- @@item Primary keys, unique, check and exclusion constraints 85/192
ALTER TABLE ONLY emergency_contacts ADD CONSTRAINT emergency_contacts_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 86/192
...
-- @@item Primary keys, unique, check and exclusion constraints 87/192
ALTER TABLE ONLY endorsements ADD CONSTRAINT endorsements_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 88/192
...
-- @@item Primary keys, unique, check and exclusion constraints 89/192
ALTER TABLE ONLY entity_password_attempts ADD CONSTRAINT entity_password_attempts_pkey PRIMARY KEY (owner_user_id, kind);

-- @@item Primary keys, unique, check and exclusion constraints 90/192
...
-- @@item Primary keys, unique, check and exclusion constraints 91/192
ALTER TABLE ONLY entity_recovery_attempts ADD CONSTRAINT entity_recovery_attempts_pkey PRIMARY KEY (owner_user_id, kind);

-- @@item Primary keys, unique, check and exclusion constraints 92/192
...
-- @@item Primary keys, unique, check and exclusion constraints 97/192
ALTER TABLE ONLY gate_passes ADD CONSTRAINT gate_passes_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 98/192
...
-- @@item Primary keys, unique, check and exclusion constraints 117/192
ALTER TABLE ONLY payments ADD CONSTRAINT payments_status_check CHECK ((status = ANY (ARRAY['CREATED'::text, 'PAID'::text, 'FAILED'::text])));

-- @@item Primary keys, unique, check and exclusion constraints 118/192
ALTER TABLE ONLY places ADD CONSTRAINT places_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 119/192
...
-- @@item Primary keys, unique, check and exclusion constraints 120/192
...
-- @@item Primary keys, unique, check and exclusion constraints 121/192
ALTER TABLE ONLY poll_votes ADD CONSTRAINT poll_votes_pkey PRIMARY KEY (post_id, user_id);

-- @@item Primary keys, unique, check and exclusion constraints 122/192
ALTER TABLE ONLY portfolio_items ADD CONSTRAINT portfolio_items_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 123/192
ALTER TABLE ONLY post_comments ADD CONSTRAINT post_comments_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 124/192
...
-- @@item Primary keys, unique, check and exclusion constraints 125/192
ALTER TABLE ONLY post_likes ADD CONSTRAINT post_likes_pkey PRIMARY KEY (post_id, user_id);

-- @@item Primary keys, unique, check and exclusion constraints 126/192
ALTER TABLE ONLY post_saves ADD CONSTRAINT post_saves_pkey PRIMARY KEY (post_id, user_id);

-- @@item Primary keys, unique, check and exclusion constraints 127/192
ALTER TABLE ONLY private.admin_action_logs ADD CONSTRAINT admin_action_logs_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 128/192
ALTER TABLE ONLY pro_payments ADD CONSTRAINT pro_payments_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 129/192
...
-- @@item Primary keys, unique, check and exclusion constraints 130/192
...
-- @@item Primary keys, unique, check and exclusion constraints 131/192
...
-- @@item Primary keys, unique, check and exclusion constraints 132/192
ALTER TABLE ONLY profile_deletion_requests ADD CONSTRAINT profile_deletion_requests_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 133/192
...
-- @@item Primary keys, unique, check and exclusion constraints 142/192
ALTER TABLE ONLY providers ADD CONSTRAINT providers_package_key_check CHECK (((package_key IS NULL) OR (package_key = ANY (ARRAY['clinic'::text, 'diagnostics'::text, 'vet'::text, 'pharmacy'::text, 'dining'::text, 'takeaway'::text, 'salon'::text, 'shop'::text, 'homeservice'::text, 'learning'::text, 'fitness'::text, 'professional'::text, 'events'::text, 'generic'::text]))));

-- @@item Primary keys, unique, check and exclusion constraints 143/192
...
-- @@item Primary keys, unique, check and exclusion constraints 151/192
ALTER TABLE ONLY ratings ADD CONSTRAINT ratings_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 152/192
ALTER TABLE ONLY ratings ADD CONSTRAINT ratings_rating_check CHECK (((rating >= 1) AND (rating <= 5)));

-- @@item Primary keys, unique, check and exclusion constraints 153/192
ALTER TABLE ONLY reports ADD CONSTRAINT reports_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 154/192
...
-- @@item Primary keys, unique, check and exclusion constraints 172/192
...
-- @@item Primary keys, unique, check and exclusion constraints 173/192
ALTER TABLE ONLY subscriptions ADD CONSTRAINT subscriptions_pkey PRIMARY KEY (id);

-- @@item Primary keys, unique, check and exclusion constraints 174/192
...
-- @@item Primary keys, unique, check and exclusion constraints 175/192
...
-- @@item Indexes (not backing a constraint) 21/178
CREATE INDEX appointments_target_idx ON public.appointments USING btree (target_id);

-- @@item Indexes (not backing a constraint) 22/178
...
-- @@item Indexes (not backing a constraint) 26/178
...
-- @@item Indexes (not backing a constraint) 38/178
...
-- @@item Indexes (not backing a constraint) 60/178
CREATE INDEX community_posts_geom_idx ON public.community_posts USING gist (geom);

-- @@item Indexes (not backing a constraint) 61/178
...
-- @@item Indexes (not backing a constraint) 62/178
...
-- @@item Indexes (not backing a constraint) 63/178
...
-- @@item Indexes (not backing a constraint) 123/178
CREATE INDEX proposals_request_idx ON public.proposals USING btree (request_id);

-- @@item Indexes (not backing a constraint) 124/178
...
-- @@item Indexes (not backing a constraint) 137/178
...
-- @@item Indexes (not backing a constraint) 138/178
...
-- @@item Triggers 62/69
CREATE TRIGGER me_too_count_trigger AFTER INSERT OR DELETE ON request_me_toos FOR EACH ROW EXECUTE FUNCTION sync_request_me_too();
...
-- @@item Policies 145/206
CREATE POLICY queue_tokens_select_participants ON public.queue_tokens AS PERMISSIVE FOR SELECT TO authenticated
  USING (((customer_user_id = (( SELECT auth.uid() AS uid))::text) OR (EXISTS ( SELECT 1
   FROM businesses b
  WHERE ((b.id = queue_tokens.business_id) AND (b.owner_user_id = (( SELECT auth.uid() AS uid))::text))))));

-- @@item Policies 146/206
...
-- @@item Policies 168/206
CREATE POLICY stories_delete_own ON public.stories AS PERMISSIVE FOR DELETE TO authenticated
  USING ((((owner_type = 'user'::text) AND (user_id = (( SELECT auth.uid() AS uid))::text)) OR ((owner_type = 'business'::text) AND can_manage_business(owner_id)) OR ((owner_type = 'provider'::text) AND (EXISTS ( SELECT 1
   FROM providers p
  WHERE ((p.id = stories.owner_id) AND (p.user_id = (( SELECT auth.uid() AS uid))::text)))))));

-- @@item Policies 169/206
CREATE POLICY stories_insert_own ON public.stories AS PERMISSIVE FOR INSERT TO authenticated
  WITH CHECK (((user_id = (( SELECT auth.uid() AS uid))::text) AND (((owner_type = 'user'::text) AND (owner_id = (( SELECT auth.uid() AS uid))::text)) OR ((owner_type = 'business'::text) AND can_manage_business(owner_id)) OR ((owner_type = 'provider'::text) AND (EXISTS ( SELECT 1
   FROM providers p
  WHERE ((p.id = stories.owner_id) AND (p.user_id = (( SELECT auth.uid() AS uid))::text))))))));

-- @@item Policies 170/206
CREATE POLICY stories_select ON public.stories AS PERMISSIVE FOR SELECT TO PUBLIC
...
-- @@item Policies 171/206
CREATE POLICY stories_update_own ON public.stories AS PERMISSIVE FOR UPDATE TO authenticated
  USING ((((owner_type = 'user'::text) AND (user_id = (( SELECT auth.uid() AS uid))::text)) OR ((owner_type = 'business'::text) AND can_manage_business(owner_id)) OR ((owner_type = 'provider'::text) AND (EXISTS ( SELECT 1
   FROM providers p
  WHERE ((p.id = stories.owner_id) AND (p.user_id = (( SELECT auth.uid() AS uid))::text)))))))
  WITH CHECK ((((owner_type = 'user'::text) AND (user_id = (( SELECT auth.uid() AS uid))::text)) OR ((owner_type = 'business'::text) AND can_manage_business(owner_id)) OR ((owner_type = 'provider'::text) AND (EXISTS ( SELECT 1
   FROM providers p
  WHERE ((p.id = stories.owner_id) AND (p.user_id = (( SELECT auth.uid() AS uid))::text)))))));

-- @@item Policies 172/206
...
-- @@item Policies 204/206
CREATE POLICY del_vouches ON public.vouches AS PERMISSIVE FOR DELETE TO PUBLIC
  USING ((from_user_id = (( SELECT auth.uid() AS uid))::text));

-- @@item Policies 205/206
CREATE POLICY ins_vouches ON public.vouches AS PERMISSIVE FOR INSERT TO PUBLIC
  WITH CHECK ((from_user_id = (( SELECT auth.uid() AS uid))::text));

-- @@item Policies 206/206
CREATE POLICY read_vouches ON public.vouches AS PERMISSIVE FOR SELECT TO PUBLIC
  USING (true);

-- @@item Table and view grants (exact: revoke defaults, then grant) 1/91
REVOKE ALL ON TABLE private.admin_action_logs FROM anon, authenticated, service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 2/91
REVOKE ALL ON TABLE public.account_appeals FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.account_appeals TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.account_appeals TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.account_appeals TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 3/91
REVOKE ALL ON TABLE public.admin_login_resolve_attempts FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.admin_login_resolve_attempts TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 4/91
REVOKE ALL ON TABLE public.agreements FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.agreements TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.agreements TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 5/91
REVOKE ALL ON TABLE public.appointment_deliveries FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, TRIGGER, TRUNCATE, UPDATE ON TABLE public.appointment_deliveries TO anon;
GRANT DELETE, INSERT, REFERENCES, TRIGGER, TRUNCATE, UPDATE ON TABLE public.appointment_deliveries TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.appointment_deliveries TO service_role;
GRANT SELECT (id, appointment_id, business_id, agent_user_id, status, handoff_verified, lat, lng, live_status, created_at, delivered_at, batch_id, stop_order, cancelled_at, cancelled_by, cancel_reason, cancel_note) ON TABLE public.appointment_deliveries TO authenticated;

-- @@item Table and view grants (exact: revoke defaults, then grant) 6/91
REVOKE ALL ON TABLE public.appointment_items FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.appointment_items TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.appointment_items TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 7/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 8/91
REVOKE ALL ON TABLE public.blocked_slots FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.blocked_slots TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.blocked_slots TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.blocked_slots TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 9/91
REVOKE ALL ON TABLE public.bookmarks FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bookmarks TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bookmarks TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bookmarks TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 10/91
REVOKE ALL ON TABLE public.boosts FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.boosts TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.boosts TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.boosts TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 11/91
REVOKE ALL ON TABLE public.bug_reports FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bug_reports TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bug_reports TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bug_reports TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 12/91
REVOKE ALL ON TABLE public.bulk_deal_pledges FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bulk_deal_pledges TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bulk_deal_pledges TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bulk_deal_pledges TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 13/91
REVOKE ALL ON TABLE public.bulk_deal_tokens FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bulk_deal_tokens TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bulk_deal_tokens TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bulk_deal_tokens TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 14/91
REVOKE ALL ON TABLE public.bulk_deals FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bulk_deals TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bulk_deals TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.bulk_deals TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 15/91
REVOKE ALL ON TABLE public.business_access_sessions FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_access_sessions TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_access_sessions TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 16/91
REVOKE ALL ON TABLE public.business_login_attempts FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_login_attempts TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 17/91
REVOKE ALL ON TABLE public.business_login_credentials FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_login_credentials TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_login_credentials TO service_role;
GRANT SELECT (business_id, login_id, require_approval, session_hours, is_enabled, created_at, updated_at) ON TABLE public.business_login_credentials TO authenticated;

-- @@item Table and view grants (exact: revoke defaults, then grant) 18/91
REVOKE ALL ON TABLE public.business_packages FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_packages TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_packages TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_packages TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 19/91
REVOKE ALL ON TABLE public.business_portfolio_items FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_portfolio_items TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_portfolio_items TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_portfolio_items TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 20/91
REVOKE ALL ON TABLE public.business_qna FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_qna TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_qna TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_qna TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 21/91
REVOKE ALL ON TABLE public.business_team_members FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_team_members TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_team_members TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_team_members TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 22/91
REVOKE ALL ON TABLE public.business_view_logs FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_view_logs TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_view_logs TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.business_view_logs TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 23/91
REVOKE ALL ON TABLE public.businesses FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.businesses TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.businesses TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.businesses TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 24/91
REVOKE ALL ON TABLE public.catalog_items FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.catalog_items TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.catalog_items TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.catalog_items TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 25/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 26/91
REVOKE ALL ON TABLE public.client_errors FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.client_errors TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.client_errors TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.client_errors TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 27/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 28/91
REVOKE ALL ON TABLE public.community_posts FROM anon, authenticated, service_role;
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 29/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 30/91
REVOKE ALL ON TABLE public.custom_payments FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.custom_payments TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.custom_payments TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.custom_payments TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 31/91
REVOKE ALL ON TABLE public.delivery_agent_duty FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.delivery_agent_duty TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.delivery_agent_duty TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.delivery_agent_duty TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 32/91
REVOKE ALL ON TABLE public.delivery_batches FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.delivery_batches TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.delivery_batches TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.delivery_batches TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 33/91
REVOKE ALL ON TABLE public.emergency_contacts FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.emergency_contacts TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.emergency_contacts TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.emergency_contacts TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 34/91
REVOKE ALL ON TABLE public.endorsements FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.endorsements TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.endorsements TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.endorsements TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 35/91
REVOKE ALL ON TABLE public.entity_password_attempts FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.entity_password_attempts TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 36/91
REVOKE ALL ON TABLE public.entity_recovery_attempts FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.entity_recovery_attempts TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 37/91
REVOKE ALL ON TABLE public.fcm_tokens FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.fcm_tokens TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.fcm_tokens TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.fcm_tokens TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 38/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 39/91
REVOKE ALL ON TABLE public.gate_passes FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.gate_passes TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.gate_passes TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.gate_passes TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 40/91
REVOKE ALL ON TABLE public.group_buy_tokens FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.group_buy_tokens TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.group_buy_tokens TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.group_buy_tokens TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 41/91
REVOKE ALL ON TABLE public.handoff_attempts FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.handoff_attempts TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 42/91
REVOKE ALL ON TABLE public.leaderboard_points FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.leaderboard_points TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.leaderboard_points TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.leaderboard_points TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 43/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 44/91
REVOKE ALL ON TABLE public.live_share_recipients FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.live_share_recipients TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.live_share_recipients TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.live_share_recipients TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 45/91
REVOKE ALL ON TABLE public.live_shares FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.live_shares TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.live_shares TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.live_shares TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 46/91
REVOKE ALL ON TABLE public.location_share_grants FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.location_share_grants TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.location_share_grants TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.location_share_grants TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 47/91
REVOKE ALL ON TABLE public.loyalty_cards FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.loyalty_cards TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.loyalty_cards TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.loyalty_cards TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 48/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 49/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 50/91
REVOKE ALL ON TABLE public.offers FROM anon, authenticated, service_role;
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 51/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 52/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 53/91
...
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.poll_votes TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 54/91
REVOKE ALL ON TABLE public.portfolio_items FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.portfolio_items TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.portfolio_items TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.portfolio_items TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 55/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 56/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 57/91
REVOKE ALL ON TABLE public.post_saves FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.post_saves TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.post_saves TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.post_saves TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 58/91
REVOKE ALL ON TABLE public.pro_payments FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.pro_payments TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.pro_payments TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.pro_payments TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 59/91
REVOKE ALL ON TABLE public.profile_deletion_requests FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.profile_deletion_requests TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.profile_deletion_requests TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.profile_deletion_requests TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 60/91
REVOKE ALL ON TABLE public.proposal_counters FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.proposal_counters TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.proposal_counters TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 61/91
REVOKE ALL ON TABLE public.proposals FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.proposals TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.proposals TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.proposals TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 62/91
REVOKE ALL ON TABLE public.provider_packages FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.provider_packages TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.provider_packages TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.provider_packages TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 63/91
REVOKE ALL ON TABLE public.provider_view_logs FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.provider_view_logs TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.provider_view_logs TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.provider_view_logs TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 64/91
...
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.providers TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.providers TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 65/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 66/91
REVOKE ALL ON TABLE public.qna_upvotes FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.qna_upvotes TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.qna_upvotes TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.qna_upvotes TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 67/91
REVOKE ALL ON TABLE public.queue_settings FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.queue_settings TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.queue_settings TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.queue_settings TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 68/91
REVOKE ALL ON TABLE public.queue_tokens FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.queue_tokens TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.queue_tokens TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 69/91
REVOKE ALL ON TABLE public.ratings FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.ratings TO anon;
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 70/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 71/91
REVOKE ALL ON TABLE public.request_me_toos FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.request_me_toos TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.request_me_toos TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.request_me_toos TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 72/91
REVOKE ALL ON TABLE public.requests FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.requests TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.requests TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.requests TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 73/91
REVOKE ALL ON TABLE public.saved_searches FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.saved_searches TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.saved_searches TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.saved_searches TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 74/91
REVOKE ALL ON TABLE public.settlements FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.settlements TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.settlements TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.settlements TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 75/91
REVOKE ALL ON TABLE public.societies FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.societies TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.societies TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.societies TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 76/91
REVOKE ALL ON TABLE public.society_members FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.society_members TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.society_members TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.society_members TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 77/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 78/91
REVOKE ALL ON TABLE public.story_views FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.story_views TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.story_views TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.story_views TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 79/91
REVOKE ALL ON TABLE public.subscription_logs FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.subscription_logs TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.subscription_logs TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.subscription_logs TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 80/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 81/91
REVOKE ALL ON TABLE public.support_tickets FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.support_tickets TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.support_tickets TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.support_tickets TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 82/91
REVOKE ALL ON TABLE public.switch_pin_attempts FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.switch_pin_attempts TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 83/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 84/91
REVOKE ALL ON TABLE public.tracking_tokens FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.tracking_tokens TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.tracking_tokens TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.tracking_tokens TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 85/91
...
-- @@item Table and view grants (exact: revoke defaults, then grant) 86/91
REVOKE ALL ON TABLE public.user_list_items FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.user_list_items TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.user_list_items TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.user_list_items TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 87/91
REVOKE ALL ON TABLE public.user_lists FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.user_lists TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.user_lists TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.user_lists TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 88/91
REVOKE ALL ON TABLE public.user_saved_coupons FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.user_saved_coupons TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.user_saved_coupons TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.user_saved_coupons TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 89/91
REVOKE ALL ON TABLE public.user_stamps FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.user_stamps TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.user_stamps TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.user_stamps TO service_role;

-- @@item Table and view grants (exact: revoke defaults, then grant) 90/91
REVOKE ALL ON TABLE public.users FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, TRIGGER, TRUNCATE, UPDATE ON TABLE public.users TO anon;
GRANT DELETE, INSERT, REFERENCES, TRIGGER, TRUNCATE, UPDATE ON TABLE public.users TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.users TO service_role;
GRANT SELECT (id, name, avatar, roles, area, city, rating_avg, rating_count, language, notification_radius_km, created_at, society_id, alias, customer_enabled, customer_deleted_at, show_posts_publicly, show_asks_publicly, show_badges_publicly, show_phone_publicly, show_city_publicly, show_rating_publicly, show_email_publicly, location_public, onboarding_completed_at, show_name_publicly, terms_accepted_version, terms_accepted_at, notif_new_business, notif_nearby_requests, notif_offers, notif_silent, notif_quiet_hours, timezone, interest_category_ids, notif_nearby_alerts) ON TABLE public.users TO anon;
GRANT SELECT (id, name, phone, avatar, roles, area, city, rating_avg, rating_count, language, notification_radius_km, created_at, society_id, alias, customer_enabled, customer_deleted_at, show_posts_publicly, show_asks_publicly, show_badges_publicly, show_phone_publicly, show_city_publicly, show_rating_publicly, show_email_publicly, location_public, onboarding_completed_at, show_name_publicly, terms_accepted_version, terms_accepted_at, notif_new_business, notif_nearby_requests, notif_offers, notif_silent, notif_quiet_hours, timezone, interest_category_ids, notif_nearby_alerts) ON TABLE public.users TO authenticated;

-- @@item Table and view grants (exact: revoke defaults, then grant) 91/91
REVOKE ALL ON TABLE public.vouches FROM anon, authenticated, service_role;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.vouches TO anon;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.vouches TO authenticated;
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.vouches TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 1/248
REVOKE ALL ON FUNCTION public._bulk_deal_close_internal(p_deal_id text, p_trigger text, p_outcome text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public._bulk_deal_close_internal(p_deal_id text, p_trigger text, p_outcome text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 2/248
REVOKE ALL ON FUNCTION public._enforce_business_owner_limit() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public._enforce_business_owner_limit() TO authenticated;
GRANT EXECUTE ON FUNCTION public._enforce_business_owner_limit() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 3/248
REVOKE ALL ON FUNCTION public._normalize_recovery_answer(p_answer text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public._normalize_recovery_answer(p_answer text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 4/248
REVOKE ALL ON FUNCTION public._user_blocks_exists(p_a text, p_b text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public._user_blocks_exists(p_a text, p_b text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 5/248
REVOKE ALL ON FUNCTION public._validate_recovery_question(p_kind text, p_question_id text, p_question_text text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public._validate_recovery_question(p_kind text, p_question_id text, p_question_text text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 6/248
REVOKE ALL ON FUNCTION public._verify_entity_password(p_kind text, p_owner_user_id text, p_password text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public._verify_entity_password(p_kind text, p_owner_user_id text, p_password text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 7/248
REVOKE ALL ON FUNCTION public._verify_entity_recovery_answer(p_kind text, p_owner_user_id text, p_answer text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public._verify_entity_recovery_answer(p_kind text, p_owner_user_id text, p_answer text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 8/248
REVOKE ALL ON FUNCTION public.accept_delivery_batch(p_batch_id text, p_stop_order text[]) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.accept_delivery_batch(p_batch_id text, p_stop_order text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.accept_delivery_batch(p_batch_id text, p_stop_order text[]) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 9/248
REVOKE ALL ON FUNCTION public.accept_proposal(p_proposal_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.accept_proposal(p_proposal_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.accept_proposal(p_proposal_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 10/248
REVOKE ALL ON FUNCTION public.accept_proposal_at_price(p_proposal_id text, p_final_price integer) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.accept_proposal_at_price(p_proposal_id text, p_final_price integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.accept_proposal_at_price(p_proposal_id text, p_final_price integer) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 11/248
REVOKE ALL ON FUNCTION public.accept_proposal_counter(p_proposal_id text, p_counter_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.accept_proposal_counter(p_proposal_id text, p_counter_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.accept_proposal_counter(p_proposal_id text, p_counter_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 12/248
REVOKE ALL ON FUNCTION public.admin_cancel_request(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.admin_cancel_request(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_cancel_request(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 13/248
REVOKE ALL ON FUNCTION public.admin_delete_comment(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.admin_delete_comment(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_delete_comment(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 14/248
REVOKE ALL ON FUNCTION public.admin_delete_post(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.admin_delete_post(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_delete_post(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 15/248
REVOKE ALL ON FUNCTION public.admin_recent_users() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.admin_recent_users() TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_recent_users() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 16/248
REVOKE ALL ON FUNCTION public.admin_resolve_agreement_dispute(p_id text, p_resolution text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.admin_resolve_agreement_dispute(p_id text, p_resolution text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_resolve_agreement_dispute(p_id text, p_resolution text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 17/248
REVOKE ALL ON FUNCTION public.admin_search_users(term text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.admin_search_users(term text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_search_users(term text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 18/248
REVOKE ALL ON FUNCTION public.agreement_cancel(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.agreement_cancel(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agreement_cancel(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 19/248
REVOKE ALL ON FUNCTION public.agreement_claim_payment(p_id text, p_method text, p_amount integer, p_reference text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.agreement_claim_payment(p_id text, p_method text, p_amount integer, p_reference text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agreement_claim_payment(p_id text, p_method text, p_amount integer, p_reference text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 20/248
REVOKE ALL ON FUNCTION public.agreement_complete(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.agreement_complete(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agreement_complete(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 21/248
REVOKE ALL ON FUNCTION public.agreement_confirm(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.agreement_confirm(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agreement_confirm(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 22/248
REVOKE ALL ON FUNCTION public.agreement_confirm_payment(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.agreement_confirm_payment(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agreement_confirm_payment(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 23/248
REVOKE ALL ON FUNCTION public.agreement_create_tracking_token(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.agreement_create_tracking_token(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agreement_create_tracking_token(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 24/248
REVOKE ALL ON FUNCTION public.agreement_dispute(p_id text, p_reason text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.agreement_dispute(p_id text, p_reason text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agreement_dispute(p_id text, p_reason text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 25/248
REVOKE ALL ON FUNCTION public.agreement_reject_payment(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.agreement_reject_payment(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agreement_reject_payment(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 26/248
REVOKE ALL ON FUNCTION public.agreement_start_work(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.agreement_start_work(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agreement_start_work(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 27/248
REVOKE ALL ON FUNCTION public.agreement_submit_review(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.agreement_submit_review(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agreement_submit_review(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 28/248
REVOKE ALL ON FUNCTION public.agreement_update_live_status(p_id text, p_status text, p_lat double precision, p_lng double precision) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.agreement_update_live_status(p_id text, p_status text, p_lat double precision, p_lng double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agreement_update_live_status(p_id text, p_status text, p_lat double precision, p_lng double precision) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 29/248
REVOKE ALL ON FUNCTION public.aliases_available(p_aliases text[]) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.aliases_available(p_aliases text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.aliases_available(p_aliases text[]) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 30/248
REVOKE ALL ON FUNCTION public.appointment_accept_with_eta(p_id text, p_eta_text text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.appointment_accept_with_eta(p_id text, p_eta_text text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.appointment_accept_with_eta(p_id text, p_eta_text text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 31/248
REVOKE ALL ON FUNCTION public.appointment_claim_payment(p_id text, p_method text, p_amount numeric, p_reference text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.appointment_claim_payment(p_id text, p_method text, p_amount numeric, p_reference text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.appointment_claim_payment(p_id text, p_method text, p_amount numeric, p_reference text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 32/248
REVOKE ALL ON FUNCTION public.appointment_confirm_payment(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.appointment_confirm_payment(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.appointment_confirm_payment(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 33/248
REVOKE ALL ON FUNCTION public.appointment_create(p_target_type text, p_target_id text, p_scheduled_for timestamp with time zone, p_date_label text, p_time_label text, p_notes text, p_photo_url text, p_package_id text, p_package_name text, p_package_price numeric, p_items jsonb, p_fulfillment_type text, p_delivery_address_line text, p_delivery_lat double precision, p_delivery_lng double precision, p_requested_delivery_window text, p_party_size integer, p_target_package_key text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.appointment_create(p_target_type text, p_target_id text, p_scheduled_for timestamp with time zone, p_date_label text, p_time_label text, p_notes text, p_photo_url text, p_package_id text, p_package_name text, p_package_price numeric, p_items jsonb, p_fulfillment_type text, p_delivery_address_line text, p_delivery_lat double precision, p_delivery_lng double precision, p_requested_delivery_window text, p_party_size integer, p_target_package_key text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.appointment_create(p_target_type text, p_target_id text, p_scheduled_for timestamp with time zone, p_date_label text, p_time_label text, p_notes text, p_photo_url text, p_package_id text, p_package_name text, p_package_price numeric, p_items jsonb, p_fulfillment_type text, p_delivery_address_line text, p_delivery_lat double precision, p_delivery_lng double precision, p_requested_delivery_window text, p_party_size integer, p_target_package_key text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 34/248
REVOKE ALL ON FUNCTION public.appointment_create_tracking_token(p_appointment_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.appointment_create_tracking_token(p_appointment_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.appointment_create_tracking_token(p_appointment_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 35/248
REVOKE ALL ON FUNCTION public.appointment_create_walk_in(p_target_type text, p_target_id text, p_customer_name text, p_customer_phone text, p_scheduled_for timestamp with time zone, p_date_label text, p_time_label text, p_package_id text, p_package_name text, p_package_price numeric, p_items jsonb, p_party_size integer, p_target_package_key text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.appointment_create_walk_in(p_target_type text, p_target_id text, p_customer_name text, p_customer_phone text, p_scheduled_for timestamp with time zone, p_date_label text, p_time_label text, p_package_id text, p_package_name text, p_package_price numeric, p_items jsonb, p_party_size integer, p_target_package_key text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.appointment_create_walk_in(p_target_type text, p_target_id text, p_customer_name text, p_customer_phone text, p_scheduled_for timestamp with time zone, p_date_label text, p_time_label text, p_package_id text, p_package_name text, p_package_price numeric, p_items jsonb, p_party_size integer, p_target_package_key text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 36/248
REVOKE ALL ON FUNCTION public.appointment_create_walk_in_payment(p_target_id text, p_package_name text, p_package_price numeric, p_method text, p_reference text, p_items jsonb) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.appointment_create_walk_in_payment(p_target_id text, p_package_name text, p_package_price numeric, p_method text, p_reference text, p_items jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.appointment_create_walk_in_payment(p_target_id text, p_package_name text, p_package_price numeric, p_method text, p_reference text, p_items jsonb) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 37/248
REVOKE ALL ON FUNCTION public.appointment_record_walk_in_payment(p_id text, p_method text, p_amount numeric, p_reference text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.appointment_record_walk_in_payment(p_id text, p_method text, p_amount numeric, p_reference text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.appointment_record_walk_in_payment(p_id text, p_method text, p_amount numeric, p_reference text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 38/248
REVOKE ALL ON FUNCTION public.appointment_reject_payment(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.appointment_reject_payment(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.appointment_reject_payment(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 39/248
REVOKE ALL ON FUNCTION public.appointment_set_unpaid_amount(p_id text, p_amount numeric) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.appointment_set_unpaid_amount(p_id text, p_amount numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.appointment_set_unpaid_amount(p_id text, p_amount numeric) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 40/248
REVOKE ALL ON FUNCTION public.appointment_transition(p_id text, p_status text, p_response_note text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.appointment_transition(p_id text, p_status text, p_response_note text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.appointment_transition(p_id text, p_status text, p_response_note text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 41/248
REVOKE ALL ON FUNCTION public.appointment_update_delivery_status(p_delivery_id text, p_status text, p_lat double precision, p_lng double precision) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.appointment_update_delivery_status(p_delivery_id text, p_status text, p_lat double precision, p_lng double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.appointment_update_delivery_status(p_delivery_id text, p_status text, p_lat double precision, p_lng double precision) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 42/248
REVOKE ALL ON FUNCTION public.assign_delivery(p_appointment_id text, p_agent_user_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.assign_delivery(p_appointment_id text, p_agent_user_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.assign_delivery(p_appointment_id text, p_agent_user_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 43/248
REVOKE ALL ON FUNCTION public.assign_delivery_batch(p_appointment_ids text[], p_agent_user_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.assign_delivery_batch(p_appointment_ids text[], p_agent_user_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.assign_delivery_batch(p_appointment_ids text[], p_agent_user_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 44/248
REVOKE ALL ON FUNCTION public.booked_slots(p_target_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.booked_slots(p_target_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.booked_slots(p_target_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 45/248
REVOKE ALL ON FUNCTION public.broadcast_new_listing(p_type text, p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.broadcast_new_listing(p_type text, p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.broadcast_new_listing(p_type text, p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 46/248
REVOKE ALL ON FUNCTION public.broadcast_offer_to_nearby(p_offer_id text, p_radius_km double precision) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.broadcast_offer_to_nearby(p_offer_id text, p_radius_km double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.broadcast_offer_to_nearby(p_offer_id text, p_radius_km double precision) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 47/248
REVOKE ALL ON FUNCTION public.bulk_deal_close(p_deal_id text, p_outcome text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bulk_deal_close(p_deal_id text, p_outcome text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bulk_deal_close(p_deal_id text, p_outcome text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 48/248
REVOKE ALL ON FUNCTION public.bulk_deal_delete(p_deal_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bulk_deal_delete(p_deal_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bulk_deal_delete(p_deal_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 49/248
REVOKE ALL ON FUNCTION public.bulk_deal_extend(p_deal_id text, p_new_closes_at timestamp with time zone) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bulk_deal_extend(p_deal_id text, p_new_closes_at timestamp with time zone) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bulk_deal_extend(p_deal_id text, p_new_closes_at timestamp with time zone) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 50/248
REVOKE ALL ON FUNCTION public.bulk_deal_pledge_claim_deposit(p_deal_id text, p_method text, p_reference text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bulk_deal_pledge_claim_deposit(p_deal_id text, p_method text, p_reference text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bulk_deal_pledge_claim_deposit(p_deal_id text, p_method text, p_reference text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 51/248
REVOKE ALL ON FUNCTION public.bulk_deal_pledge_confirm_deposit(p_deal_id text, p_pledger_user_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bulk_deal_pledge_confirm_deposit(p_deal_id text, p_pledger_user_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bulk_deal_pledge_confirm_deposit(p_deal_id text, p_pledger_user_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 52/248
REVOKE ALL ON FUNCTION public.bulk_deal_pledge_join(p_deal_id text, p_quantity integer, p_notes text, p_delivery_address text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bulk_deal_pledge_join(p_deal_id text, p_quantity integer, p_notes text, p_delivery_address text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bulk_deal_pledge_join(p_deal_id text, p_quantity integer, p_notes text, p_delivery_address text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 53/248
REVOKE ALL ON FUNCTION public.bulk_deal_pledge_leave(p_deal_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bulk_deal_pledge_leave(p_deal_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bulk_deal_pledge_leave(p_deal_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 54/248
REVOKE ALL ON FUNCTION public.bulk_deal_pledge_reject_deposit(p_deal_id text, p_pledger_user_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bulk_deal_pledge_reject_deposit(p_deal_id text, p_pledger_user_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bulk_deal_pledge_reject_deposit(p_deal_id text, p_pledger_user_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 55/248
REVOKE ALL ON FUNCTION public.bulk_deal_redemption_stats(p_deal_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bulk_deal_redemption_stats(p_deal_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bulk_deal_redemption_stats(p_deal_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 56/248
REVOKE ALL ON FUNCTION public.bulk_deal_token_redeem(p_token_code text, p_business_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bulk_deal_token_redeem(p_token_code text, p_business_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bulk_deal_token_redeem(p_token_code text, p_business_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 57/248
REVOKE ALL ON FUNCTION public.bulk_deal_tokens_for_deal(p_deal_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bulk_deal_tokens_for_deal(p_deal_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bulk_deal_tokens_for_deal(p_deal_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 58/248
REVOKE ALL ON FUNCTION public.bump_business_metric(p_business_id text, p_metric text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bump_business_metric(p_business_id text, p_metric text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bump_business_metric(p_business_id text, p_metric text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 59/248
REVOKE ALL ON FUNCTION public.bump_provider_views(p_provider_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bump_provider_views(p_provider_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.bump_provider_views(p_provider_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 60/248
REVOKE ALL ON FUNCTION public.bump_queue_line_changed() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.bump_queue_line_changed() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 61/248
REVOKE ALL ON FUNCTION public.business_active_deliveries(p_business_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.business_active_deliveries(p_business_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.business_active_deliveries(p_business_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 62/248
REVOKE ALL ON FUNCTION public.business_login_attempt(p_login_id text, p_password text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.business_login_attempt(p_login_id text, p_password text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.business_login_attempt(p_login_id text, p_password text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 63/248
REVOKE ALL ON FUNCTION public.business_slot_capacities(p_business_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.business_slot_capacities(p_business_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.business_slot_capacities(p_business_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 64/248
REVOKE ALL ON FUNCTION public.businesses_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_category text, in_limit integer, in_offset integer, in_category_ids text[]) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.businesses_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_category text, in_limit integer, in_offset integer, in_category_ids text[]) TO anon;
GRANT EXECUTE ON FUNCTION public.businesses_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_category text, in_limit integer, in_offset integer, in_category_ids text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.businesses_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_category text, in_limit integer, in_offset integer, in_category_ids text[]) TO service_role;
GRANT EXECUTE ON FUNCTION public.businesses_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_category text, in_limit integer, in_offset integer, in_category_ids text[]) TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 65/248
REVOKE ALL ON FUNCTION public.can_comment_on_post(p_post_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.can_comment_on_post(p_post_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.can_comment_on_post(p_post_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 66/248
REVOKE ALL ON FUNCTION public.can_manage_business(p_business_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.can_manage_business(p_business_id text) TO anon;
GRANT EXECUTE ON FUNCTION public.can_manage_business(p_business_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.can_manage_business(p_business_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 67/248
REVOKE ALL ON FUNCTION public.can_react_to_comment(p_comment_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.can_react_to_comment(p_comment_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.can_react_to_comment(p_comment_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 68/248
REVOKE ALL ON FUNCTION public.cancel_delivery(p_delivery_id text, p_reason text, p_note text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.cancel_delivery(p_delivery_id text, p_reason text, p_note text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.cancel_delivery(p_delivery_id text, p_reason text, p_note text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 69/248
REVOKE ALL ON FUNCTION public.cancel_expired_agreements() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.cancel_expired_agreements() TO authenticated;
GRANT EXECUTE ON FUNCTION public.cancel_expired_agreements() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 70/248
REVOKE ALL ON FUNCTION public.check_bulk_deal_target_and_close(p_deal_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.check_bulk_deal_target_and_close(p_deal_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 71/248
REVOKE ALL ON FUNCTION public.check_self_vouch() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.check_self_vouch() TO authenticated;
GRANT EXECUTE ON FUNCTION public.check_self_vouch() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 72/248
REVOKE ALL ON FUNCTION public.claim_first_admin(p_login_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.claim_first_admin(p_login_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.claim_first_admin(p_login_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 73/248
REVOKE ALL ON FUNCTION public.clear_entity_password(p_kind text, p_current_password text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.clear_entity_password(p_kind text, p_current_password text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.clear_entity_password(p_kind text, p_current_password text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 74/248
REVOKE ALL ON FUNCTION public.clear_switch_pin(p_current_pin text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.clear_switch_pin(p_current_pin text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.clear_switch_pin(p_current_pin text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 75/248
REVOKE ALL ON FUNCTION public.close_expired_bulk_deals() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.close_expired_bulk_deals() TO authenticated;
GRANT EXECUTE ON FUNCTION public.close_expired_bulk_deals() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 76/248
REVOKE ALL ON FUNCTION public.close_expired_business_sessions() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.close_expired_business_sessions() TO authenticated;
GRANT EXECUTE ON FUNCTION public.close_expired_business_sessions() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 77/248
REVOKE ALL ON FUNCTION public.close_expired_requests() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.close_expired_requests() TO authenticated;
GRANT EXECUTE ON FUNCTION public.close_expired_requests() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 78/248
REVOKE ALL ON FUNCTION public.close_stale_queue_tokens() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.close_stale_queue_tokens() TO authenticated;
GRANT EXECUTE ON FUNCTION public.close_stale_queue_tokens() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 79/248
REVOKE ALL ON FUNCTION public.comment_gate_reason(p_post_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.comment_gate_reason(p_post_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.comment_gate_reason(p_post_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 80/248
REVOKE ALL ON FUNCTION public.community_comment_delete(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_comment_delete(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_comment_delete(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 81/248
REVOKE ALL ON FUNCTION public.community_comment_set_pinned(p_id text, p_pinned boolean) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_comment_set_pinned(p_id text, p_pinned boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_comment_set_pinned(p_id text, p_pinned boolean) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 82/248
REVOKE ALL ON FUNCTION public.community_comment_update(p_id text, p_body text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_comment_update(p_id text, p_body text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_comment_update(p_id text, p_body text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 83/248
REVOKE ALL ON FUNCTION public.community_poll_close(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_poll_close(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_poll_close(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 84/248
REVOKE ALL ON FUNCTION public.community_post_add_recommendation(p_post_id text, p_listing_type text, p_listing_id text, p_by_name text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_post_add_recommendation(p_post_id text, p_listing_type text, p_listing_id text, p_by_name text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_post_add_recommendation(p_post_id text, p_listing_type text, p_listing_id text, p_by_name text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 85/248
REVOKE ALL ON FUNCTION public.community_post_delete(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_post_delete(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_post_delete(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 86/248
REVOKE ALL ON FUNCTION public.community_post_hot_score(in_likes integer, in_comments integer, in_created_at timestamp with time zone) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_post_hot_score(in_likes integer, in_comments integer, in_created_at timestamp with time zone) TO anon;
GRANT EXECUTE ON FUNCTION public.community_post_hot_score(in_likes integer, in_comments integer, in_created_at timestamp with time zone) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_post_hot_score(in_likes integer, in_comments integer, in_created_at timestamp with time zone) TO service_role;
GRANT EXECUTE ON FUNCTION public.community_post_hot_score(in_likes integer, in_comments integer, in_created_at timestamp with time zone) TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 87/248
REVOKE ALL ON FUNCTION public.community_post_set_profile_visibility(p_id text, p_show boolean) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_post_set_profile_visibility(p_id text, p_show boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_post_set_profile_visibility(p_id text, p_show boolean) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 88/248
REVOKE ALL ON FUNCTION public.community_post_set_resolved(p_id text, p_resolved boolean) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_post_set_resolved(p_id text, p_resolved boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_post_set_resolved(p_id text, p_resolved boolean) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 89/248
REVOKE ALL ON FUNCTION public.community_post_update(p_id text, p_title text, p_body text, p_image text, p_media text[], p_image_alt text, p_last_seen text, p_reward text, p_pickup_note text, p_tagged_listing jsonb, p_clear_tagged_listing boolean, p_comment_policy text, p_hide_like_count boolean) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_post_update(p_id text, p_title text, p_body text, p_image text, p_media text[], p_image_alt text, p_last_seen text, p_reward text, p_pickup_note text, p_tagged_listing jsonb, p_clear_tagged_listing boolean, p_comment_policy text, p_hide_like_count boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_post_update(p_id text, p_title text, p_body text, p_image text, p_media text[], p_image_alt text, p_last_seen text, p_reward text, p_pickup_note text, p_tagged_listing jsonb, p_clear_tagged_listing boolean, p_comment_policy text, p_hide_like_count boolean) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 90/248
REVOKE ALL ON FUNCTION public.community_posts_feed(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer, in_offset integer, in_type text, in_sort text, in_query text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_posts_feed(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer, in_offset integer, in_type text, in_sort text, in_query text) TO anon;
GRANT EXECUTE ON FUNCTION public.community_posts_feed(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer, in_offset integer, in_type text, in_sort text, in_query text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_posts_feed(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer, in_offset integer, in_type text, in_sort text, in_query text) TO service_role;
GRANT EXECUTE ON FUNCTION public.community_posts_feed(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer, in_offset integer, in_type text, in_sort text, in_query text) TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 91/248
REVOKE ALL ON FUNCTION public.community_posts_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer, in_offset integer) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_posts_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer, in_offset integer) TO anon;
GRANT EXECUTE ON FUNCTION public.community_posts_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer, in_offset integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_posts_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer, in_offset integer) TO service_role;
GRANT EXECUTE ON FUNCTION public.community_posts_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer, in_offset integer) TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 92/248
REVOKE ALL ON FUNCTION public.community_search_tsquery(p_q text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.community_search_tsquery(p_q text) TO anon;
GRANT EXECUTE ON FUNCTION public.community_search_tsquery(p_q text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_search_tsquery(p_q text) TO service_role;
GRANT EXECUTE ON FUNCTION public.community_search_tsquery(p_q text) TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 93/248
REVOKE ALL ON FUNCTION public.confirm_handoff(p_delivery_id text, p_code text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.confirm_handoff(p_delivery_id text, p_code text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.confirm_handoff(p_delivery_id text, p_code text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 94/248
REVOKE ALL ON FUNCTION public.count_my_active_deliveries(p_business_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.count_my_active_deliveries(p_business_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.count_my_active_deliveries(p_business_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 95/248
REVOKE ALL ON FUNCTION public.create_settlements_on_complete() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.create_settlements_on_complete() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 96/248
REVOKE ALL ON FUNCTION public.custom_payment_confirm(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.custom_payment_confirm(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.custom_payment_confirm(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 97/248
REVOKE ALL ON FUNCTION public.custom_payment_create(p_target_type text, p_target_id text, p_amount numeric, p_method text, p_note text, p_reference text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.custom_payment_create(p_target_type text, p_target_id text, p_amount numeric, p_method text, p_note text, p_reference text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.custom_payment_create(p_target_type text, p_target_id text, p_amount numeric, p_method text, p_note text, p_reference text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 98/248
REVOKE ALL ON FUNCTION public.custom_payment_reject(p_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.custom_payment_reject(p_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.custom_payment_reject(p_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 99/248
REVOKE ALL ON FUNCTION public.decide_business_session(p_session_id uuid, p_approve boolean) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.decide_business_session(p_session_id uuid, p_approve boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.decide_business_session(p_session_id uuid, p_approve boolean) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 100/248
REVOKE ALL ON FUNCTION public.decline_delivery_batch(p_batch_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.decline_delivery_batch(p_batch_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.decline_delivery_batch(p_batch_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 101/248
REVOKE ALL ON FUNCTION public.delete_business(p_business_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.delete_business(p_business_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.delete_business(p_business_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 102/248
REVOKE ALL ON FUNCTION public.derive_notification_scope() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.derive_notification_scope() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 103/248
REVOKE ALL ON FUNCTION public.distance_km(in_lng double precision, in_lat double precision, row_lng double precision, row_lat double precision) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.distance_km(in_lng double precision, in_lat double precision, row_lng double precision, row_lat double precision) TO anon;
GRANT EXECUTE ON FUNCTION public.distance_km(in_lng double precision, in_lat double precision, row_lng double precision, row_lat double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.distance_km(in_lng double precision, in_lat double precision, row_lng double precision, row_lat double precision) TO service_role;
GRANT EXECUTE ON FUNCTION public.distance_km(in_lng double precision, in_lat double precision, row_lng double precision, row_lat double precision) TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 104/248
REVOKE ALL ON FUNCTION public.enforce_business_location_freeze() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.enforce_business_location_freeze() TO authenticated;
GRANT EXECUTE ON FUNCTION public.enforce_business_location_freeze() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 105/248
REVOKE ALL ON FUNCTION public.enforce_customer_daily_appointment_limit() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.enforce_customer_daily_appointment_limit() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 106/248
REVOKE ALL ON FUNCTION public.enforce_manual_verification_decision() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.enforce_manual_verification_decision() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 107/248
REVOKE ALL ON FUNCTION public.enforce_places_status_freeze() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.enforce_places_status_freeze() TO authenticated;
GRANT EXECUTE ON FUNCTION public.enforce_places_status_freeze() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 108/248
REVOKE ALL ON FUNCTION public.enforce_proposal_responder_entity_owner() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.enforce_proposal_responder_entity_owner() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 109/248
REVOKE ALL ON FUNCTION public.enforce_queue_cancel_rules() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.enforce_queue_cancel_rules() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 110/248
REVOKE ALL ON FUNCTION public.enforce_queue_open_on_join() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.enforce_queue_open_on_join() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 111/248
REVOKE ALL ON FUNCTION public.enforce_queue_token_customer_update() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.enforce_queue_token_customer_update() TO anon;
GRANT EXECUTE ON FUNCTION public.enforce_queue_token_customer_update() TO authenticated;
GRANT EXECUTE ON FUNCTION public.enforce_queue_token_customer_update() TO service_role;
GRANT EXECUTE ON FUNCTION public.enforce_queue_token_customer_update() TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 112/248
REVOKE ALL ON FUNCTION public.enforce_role_privilege_guard() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.enforce_role_privilege_guard() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 113/248
REVOKE ALL ON FUNCTION public.enforce_slot_capacity() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.enforce_slot_capacity() TO anon;
GRANT EXECUTE ON FUNCTION public.enforce_slot_capacity() TO authenticated;
GRANT EXECUTE ON FUNCTION public.enforce_slot_capacity() TO service_role;
GRANT EXECUTE ON FUNCTION public.enforce_slot_capacity() TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 114/248
REVOKE ALL ON FUNCTION public.expire_tokens_on_queue_close() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.expire_tokens_on_queue_close() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 115/248
REVOKE ALL ON FUNCTION public.get_delivery_duty() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_delivery_duty() TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_delivery_duty() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 116/248
REVOKE ALL ON FUNCTION public.get_entity_recovery_question(p_kind text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_entity_recovery_question(p_kind text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_entity_recovery_question(p_kind text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 117/248
REVOKE ALL ON FUNCTION public.get_leaderboard() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_leaderboard() TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_leaderboard() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 118/248
REVOKE ALL ON FUNCTION public.get_live_share(p_share_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_live_share(p_share_id text) TO anon;
GRANT EXECUTE ON FUNCTION public.get_live_share(p_share_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_live_share(p_share_id text) TO service_role;
GRANT EXECUTE ON FUNCTION public.get_live_share(p_share_id text) TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 119/248
REVOKE ALL ON FUNCTION public.get_nearby_user_ids(p_lat double precision, p_lng double precision, p_radius_km double precision) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_nearby_user_ids(p_lat double precision, p_lng double precision, p_radius_km double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_nearby_user_ids(p_lat double precision, p_lng double precision, p_radius_km double precision) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 120/248
REVOKE ALL ON FUNCTION public.get_own_coords() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_own_coords() TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_own_coords() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 121/248
...
-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 122/248
REVOKE ALL ON FUNCTION public.get_public_profile(target_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_public_profile(target_id text) TO anon;
GRANT EXECUTE ON FUNCTION public.get_public_profile(target_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_public_profile(target_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 123/248
REVOKE ALL ON FUNCTION public.get_shared_location(p_target text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_shared_location(p_target text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_shared_location(p_target text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 124/248
...
GRANT EXECUTE ON FUNCTION public.get_tracking(p_token text) TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 125/248
REVOKE ALL ON FUNCTION public.grant_business_access(p_business_id text, p_identifier text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.grant_business_access(p_business_id text, p_identifier text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.grant_business_access(p_business_id text, p_identifier text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 126/248
REVOKE ALL ON FUNCTION public.grant_team_access(p_business_id text, p_identifier text, p_scopes text[]) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.grant_team_access(p_business_id text, p_identifier text, p_scopes text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.grant_team_access(p_business_id text, p_identifier text, p_scopes text[]) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 127/248
REVOKE ALL ON FUNCTION public.grant_team_member_access(p_business_id text, p_identifier text, p_scopes text[]) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.grant_team_member_access(p_business_id text, p_identifier text, p_scopes text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.grant_team_member_access(p_business_id text, p_identifier text, p_scopes text[]) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 128/248
REVOKE ALL ON FUNCTION public.group_buy_issue_tokens(p_request_id text, p_agreement_id text, p_business_id text, p_unit_price numeric, p_valid_until timestamp with time zone) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.group_buy_issue_tokens(p_request_id text, p_agreement_id text, p_business_id text, p_unit_price numeric, p_valid_until timestamp with time zone) TO authenticated;
GRANT EXECUTE ON FUNCTION public.group_buy_issue_tokens(p_request_id text, p_agreement_id text, p_business_id text, p_unit_price numeric, p_valid_until timestamp with time zone) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 129/248
REVOKE ALL ON FUNCTION public.group_buy_join(p_request_id text, p_quantity integer, p_notes text, p_delivery_address text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.group_buy_join(p_request_id text, p_quantity integer, p_notes text, p_delivery_address text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.group_buy_join(p_request_id text, p_quantity integer, p_notes text, p_delivery_address text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 130/248
REVOKE ALL ON FUNCTION public.group_buy_leave(p_request_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.group_buy_leave(p_request_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.group_buy_leave(p_request_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 131/248
REVOKE ALL ON FUNCTION public.group_buy_redemption_stats(p_agreement_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.group_buy_redemption_stats(p_agreement_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.group_buy_redemption_stats(p_agreement_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 132/248
REVOKE ALL ON FUNCTION public.group_buy_token_redeem(p_token_code text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.group_buy_token_redeem(p_token_code text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.group_buy_token_redeem(p_token_code text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 133/248
REVOKE ALL ON FUNCTION public.group_buy_tokens_for_agreement(p_agreement_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.group_buy_tokens_for_agreement(p_agreement_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.group_buy_tokens_for_agreement(p_agreement_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 134/248
REVOKE ALL ON FUNCTION public.handle_new_auth_user() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.handle_new_auth_user() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 135/248
REVOKE ALL ON FUNCTION public.has_business_access(p_business_id text, p_uid text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.has_business_access(p_business_id text, p_uid text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.has_business_access(p_business_id text, p_uid text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 136/248
REVOKE ALL ON FUNCTION public.has_business_full_access(p_business_id text, p_uid text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.has_business_full_access(p_business_id text, p_uid text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.has_business_full_access(p_business_id text, p_uid text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 137/248
REVOKE ALL ON FUNCTION public.has_business_scope(p_business_id text, p_uid text, p_scope text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.has_business_scope(p_business_id text, p_uid text, p_scope text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.has_business_scope(p_business_id text, p_uid text, p_scope text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 138/248
REVOKE ALL ON FUNCTION public.haversine_km(lat1 double precision, lng1 double precision, lat2 double precision, lng2 double precision) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.haversine_km(lat1 double precision, lng1 double precision, lat2 double precision, lng2 double precision) TO anon;
GRANT EXECUTE ON FUNCTION public.haversine_km(lat1 double precision, lng1 double precision, lat2 double precision, lng2 double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.haversine_km(lat1 double precision, lng1 double precision, lat2 double precision, lng2 double precision) TO service_role;
GRANT EXECUTE ON FUNCTION public.haversine_km(lat1 double precision, lng1 double precision, lat2 double precision, lng2 double precision) TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 139/248
REVOKE ALL ON FUNCTION public.increment_stamp(p_card_id text, p_user_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.increment_stamp(p_card_id text, p_user_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.increment_stamp(p_card_id text, p_user_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 140/248
REVOKE ALL ON FUNCTION public.is_admin() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.is_admin() TO anon;
GRANT EXECUTE ON FUNCTION public.is_admin() TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_admin() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 141/248
REVOKE ALL ON FUNCTION public.is_admin(p_user_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.is_admin(p_user_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_admin(p_user_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 142/248
REVOKE ALL ON FUNCTION public.is_blocked_between(p_other_user_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.is_blocked_between(p_other_user_id text) TO anon;
GRANT EXECUTE ON FUNCTION public.is_blocked_between(p_other_user_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_blocked_between(p_other_user_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 143/248
REVOKE ALL ON FUNCTION public.is_entity_password_set(p_kind text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.is_entity_password_set(p_kind text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_entity_password_set(p_kind text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 144/248
REVOKE ALL ON FUNCTION public.is_entity_recovery_set(p_kind text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.is_entity_recovery_set(p_kind text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_entity_recovery_set(p_kind text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 145/248
REVOKE ALL ON FUNCTION public.is_society_admin(p_society_id uuid, p_user_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.is_society_admin(p_society_id uuid, p_user_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_society_admin(p_society_id uuid, p_user_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 146/248
REVOKE ALL ON FUNCTION public.is_society_member(p_society_id uuid, p_user_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.is_society_member(p_society_id uuid, p_user_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_society_member(p_society_id uuid, p_user_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 147/248
REVOKE ALL ON FUNCTION public.is_switch_pin_set() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.is_switch_pin_set() TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_switch_pin_set() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 148/248
REVOKE ALL ON FUNCTION public.me_too_toggle(p_request_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.me_too_toggle(p_request_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.me_too_toggle(p_request_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 149/248
REVOKE ALL ON FUNCTION public.my_business_access_scope(p_business_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.my_business_access_scope(p_business_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.my_business_access_scope(p_business_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 150/248
REVOKE ALL ON FUNCTION public.my_business_access_status(p_business_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.my_business_access_status(p_business_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.my_business_access_status(p_business_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 151/248
REVOKE ALL ON FUNCTION public.my_delegated_business_password_status() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.my_delegated_business_password_status() TO authenticated;
GRANT EXECUTE ON FUNCTION public.my_delegated_business_password_status() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 152/248
REVOKE ALL ON FUNCTION public.my_delegated_businesses() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.my_delegated_businesses() TO authenticated;
GRANT EXECUTE ON FUNCTION public.my_delegated_businesses() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 153/248
REVOKE ALL ON FUNCTION public.my_deliveries() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.my_deliveries() TO authenticated;
GRANT EXECUTE ON FUNCTION public.my_deliveries() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 154/248
REVOKE ALL ON FUNCTION public.my_delivery_progress(p_appointment_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.my_delivery_progress(p_appointment_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.my_delivery_progress(p_appointment_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 155/248
REVOKE ALL ON FUNCTION public.my_duty_blockers() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.my_duty_blockers() TO authenticated;
GRANT EXECUTE ON FUNCTION public.my_duty_blockers() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 156/248
REVOKE ALL ON FUNCTION public.my_live_share_recipients() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.my_live_share_recipients() TO authenticated;
GRANT EXECUTE ON FUNCTION public.my_live_share_recipients() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 157/248
REVOKE ALL ON FUNCTION public.neighborhood_today(in_lat double precision, in_lng double precision, in_radius_m integer) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.neighborhood_today(in_lat double precision, in_lng double precision, in_radius_m integer) TO anon;
GRANT EXECUTE ON FUNCTION public.neighborhood_today(in_lat double precision, in_lng double precision, in_radius_m integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.neighborhood_today(in_lat double precision, in_lng double precision, in_radius_m integer) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 158/248
REVOKE ALL ON FUNCTION public.notification_push_health() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notification_push_health() TO authenticated;
GRANT EXECUTE ON FUNCTION public.notification_push_health() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 159/248
REVOKE ALL ON FUNCTION public.notify_admins_business_pending() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_admins_business_pending() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 160/248
REVOKE ALL ON FUNCTION public.notify_agreement_confirm(p_agreement_id text, p_recipient_user_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_agreement_confirm(p_agreement_id text, p_recipient_user_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.notify_agreement_confirm(p_agreement_id text, p_recipient_user_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 161/248
REVOKE ALL ON FUNCTION public.notify_ended_polls() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_ended_polls() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 162/248
REVOKE ALL ON FUNCTION public.notify_on_agreement() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_agreement() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 163/248
REVOKE ALL ON FUNCTION public.notify_on_appointment_created() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_appointment_created() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 164/248
REVOKE ALL ON FUNCTION public.notify_on_appointment_status() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_appointment_status() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 165/248
REVOKE ALL ON FUNCTION public.notify_on_chat_message() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_chat_message() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 166/248
REVOKE ALL ON FUNCTION public.notify_on_comment_mention() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_comment_mention() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 167/248
REVOKE ALL ON FUNCTION public.notify_on_comment_reaction() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_comment_reaction() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 168/248
REVOKE ALL ON FUNCTION public.notify_on_nearby_alert() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_nearby_alert() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 169/248
REVOKE ALL ON FUNCTION public.notify_on_post_comment() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_post_comment() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 170/248
REVOKE ALL ON FUNCTION public.notify_on_post_like() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_post_like() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 171/248
REVOKE ALL ON FUNCTION public.notify_on_post_recommendation() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_post_recommendation() TO authenticated;
GRANT EXECUTE ON FUNCTION public.notify_on_post_recommendation() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 172/248
REVOKE ALL ON FUNCTION public.notify_on_post_resolved() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_post_resolved() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 173/248
REVOKE ALL ON FUNCTION public.notify_on_proposal() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_proposal() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 174/248
REVOKE ALL ON FUNCTION public.notify_on_proposal_broadcast() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_proposal_broadcast() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 175/248
REVOKE ALL ON FUNCTION public.notify_on_qna_answered() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_qna_answered() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 176/248
REVOKE ALL ON FUNCTION public.notify_on_qna_asked() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_qna_asked() TO authenticated;
GRANT EXECUTE ON FUNCTION public.notify_on_qna_asked() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 177/248
REVOKE ALL ON FUNCTION public.notify_on_queue_called() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_queue_called() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 178/248
REVOKE ALL ON FUNCTION public.notify_on_queue_payment_status() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_queue_payment_status() TO authenticated;
GRANT EXECUTE ON FUNCTION public.notify_on_queue_payment_status() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 179/248
REVOKE ALL ON FUNCTION public.notify_on_rating() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_rating() TO authenticated;
GRANT EXECUTE ON FUNCTION public.notify_on_rating() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 180/248
REVOKE ALL ON FUNCTION public.notify_on_report_resolved() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_report_resolved() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 181/248
REVOKE ALL ON FUNCTION public.notify_on_request() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_request() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 182/248
REVOKE ALL ON FUNCTION public.notify_on_story_reaction() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_on_story_reaction() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 183/248
REVOKE ALL ON FUNCTION public.notify_saved_search_matches_business() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_saved_search_matches_business() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 184/248
REVOKE ALL ON FUNCTION public.notify_saved_search_matches_provider() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_saved_search_matches_provider() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 185/248
REVOKE ALL ON FUNCTION public.notify_verification_decision_business() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_verification_decision_business() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 186/248
REVOKE ALL ON FUNCTION public.notify_verification_decision_provider() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.notify_verification_decision_provider() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 187/248
REVOKE ALL ON FUNCTION public.on_queue_token_update() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.on_queue_token_update() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 188/248
REVOKE ALL ON FUNCTION public.proposal_submit_counter(p_proposal_id text, p_amount numeric, p_message text) FROM PUBLIC, anon, authenticated, service_role;
...
GRANT EXECUTE ON FUNCTION public.proposal_submit_counter(p_proposal_id text, p_amount numeric, p_message text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 189/248
REVOKE ALL ON FUNCTION public.protect_business_owner() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.protect_business_owner() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 190/248
REVOKE ALL ON FUNCTION public.provider_leads(p_provider_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.provider_leads(p_provider_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.provider_leads(p_provider_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 191/248
REVOKE ALL ON FUNCTION public.providers_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_category text, in_limit integer, in_offset integer, in_category_ids text[]) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.providers_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_category text, in_limit integer, in_offset integer, in_category_ids text[]) TO anon;
GRANT EXECUTE ON FUNCTION public.providers_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_category text, in_limit integer, in_offset integer, in_category_ids text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.providers_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_category text, in_limit integer, in_offset integer, in_category_ids text[]) TO service_role;
GRANT EXECUTE ON FUNCTION public.providers_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_category text, in_limit integer, in_offset integer, in_category_ids text[]) TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 192/248
REVOKE ALL ON FUNCTION public.push_on_notification_insert() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.push_on_notification_insert() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 193/248
REVOKE ALL ON FUNCTION public.qna_to_lead() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.qna_to_lead() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 194/248
REVOKE ALL ON FUNCTION public.queue_token_create_walk_in(p_business_id text, p_customer_name text, p_party_size text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.queue_token_create_walk_in(p_business_id text, p_customer_name text, p_party_size text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.queue_token_create_walk_in(p_business_id text, p_customer_name text, p_party_size text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 195/248
REVOKE ALL ON FUNCTION public.queue_waiting_line(p_business_ids text[]) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.queue_waiting_line(p_business_ids text[]) TO anon;
GRANT EXECUTE ON FUNCTION public.queue_waiting_line(p_business_ids text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.queue_waiting_line(p_business_ids text[]) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 196/248
REVOKE ALL ON FUNCTION public.recompute_rating_aggregates() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.recompute_rating_aggregates() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 197/248
REVOKE ALL ON FUNCTION public.record_terms_acceptance(p_version text, p_user_agent text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.record_terms_acceptance(p_version text, p_user_agent text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.record_terms_acceptance(p_version text, p_user_agent text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 198/248
REVOKE ALL ON FUNCTION public.renew_location_share(p_requester text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.renew_location_share(p_requester text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.renew_location_share(p_requester text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 199/248
REVOKE ALL ON FUNCTION public.reply_to_rating(p_rating_id text, p_reply text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.reply_to_rating(p_rating_id text, p_reply text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.reply_to_rating(p_rating_id text, p_reply text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 200/248
REVOKE ALL ON FUNCTION public.request_location_share(p_owner text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.request_location_share(p_owner text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.request_location_share(p_owner text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 201/248
REVOKE ALL ON FUNCTION public.reschedule_appointment(p_original_id text, p_scheduled_for timestamp with time zone, p_date_label text, p_time_label text, p_notes text, p_photo_url text, p_package_id text, p_package_name text, p_package_price numeric) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.reschedule_appointment(p_original_id text, p_scheduled_for timestamp with time zone, p_date_label text, p_time_label text, p_notes text, p_photo_url text, p_package_id text, p_package_name text, p_package_price numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.reschedule_appointment(p_original_id text, p_scheduled_for timestamp with time zone, p_date_label text, p_time_label text, p_notes text, p_photo_url text, p_package_id text, p_package_name text, p_package_price numeric) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 202/248
REVOKE ALL ON FUNCTION public.reserve_catalog_item(p_item_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.reserve_catalog_item(p_item_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 203/248
REVOKE ALL ON FUNCTION public.reserve_catalog_items(p_items jsonb) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.reserve_catalog_items(p_items jsonb) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 204/248
REVOKE ALL ON FUNCTION public.reset_boost_reminder() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.reset_boost_reminder() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 205/248
REVOKE ALL ON FUNCTION public.reset_entity_password_via_recovery(p_kind text, p_answer text, p_new_password text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.reset_entity_password_via_recovery(p_kind text, p_answer text, p_new_password text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.reset_entity_password_via_recovery(p_kind text, p_answer text, p_new_password text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 206/248
REVOKE ALL ON FUNCTION public.resolve_admin_email(p_login_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.resolve_admin_email(p_login_id text) TO anon;
GRANT EXECUTE ON FUNCTION public.resolve_admin_email(p_login_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.resolve_admin_email(p_login_id text) TO service_role;
GRANT EXECUTE ON FUNCTION public.resolve_admin_email(p_login_id text) TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 207/248
REVOKE ALL ON FUNCTION public.resolve_slot_capacity(p_target_type text, p_target_id text, p_package_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.resolve_slot_capacity(p_target_type text, p_target_id text, p_package_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.resolve_slot_capacity(p_target_type text, p_target_id text, p_package_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 208/248
REVOKE ALL ON FUNCTION public.respond_location_share(p_requester text, p_approve boolean) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.respond_location_share(p_requester text, p_approve boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.respond_location_share(p_requester text, p_approve boolean) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 209/248
REVOKE ALL ON FUNCTION public.revoke_business_session(p_session_id uuid) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.revoke_business_session(p_session_id uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.revoke_business_session(p_session_id uuid) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 210/248
REVOKE ALL ON FUNCTION public.revoke_live_share_recipient(p_recipient_user_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.revoke_live_share_recipient(p_recipient_user_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.revoke_live_share_recipient(p_recipient_user_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 211/248
REVOKE ALL ON FUNCTION public.revoke_location_share(p_requester text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.revoke_location_share(p_requester text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.revoke_location_share(p_requester text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 212/248
REVOKE ALL ON FUNCTION public.rls_auto_enable() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.rls_auto_enable() TO authenticated;
GRANT EXECUTE ON FUNCTION public.rls_auto_enable() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 213/248
REVOKE ALL ON FUNCTION public.set_admin_login_id(p_new_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.set_admin_login_id(p_new_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.set_admin_login_id(p_new_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 214/248
REVOKE ALL ON FUNCTION public.set_business_login(p_business_id text, p_login_id text, p_password text, p_require_approval boolean, p_session_hours integer, p_enabled boolean) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.set_business_login(p_business_id text, p_login_id text, p_password text, p_require_approval boolean, p_session_hours integer, p_enabled boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.set_business_login(p_business_id text, p_login_id text, p_password text, p_require_approval boolean, p_session_hours integer, p_enabled boolean) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 215/248
REVOKE ALL ON FUNCTION public.set_default_user_alias() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.set_default_user_alias() TO anon;
GRANT EXECUTE ON FUNCTION public.set_default_user_alias() TO authenticated;
GRANT EXECUTE ON FUNCTION public.set_default_user_alias() TO service_role;
GRANT EXECUTE ON FUNCTION public.set_default_user_alias() TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 216/248
REVOKE ALL ON FUNCTION public.set_delivery_duty(p_on_duty boolean) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.set_delivery_duty(p_on_duty boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.set_delivery_duty(p_on_duty boolean) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 217/248
REVOKE ALL ON FUNCTION public.set_entity_password(p_kind text, p_new_password text, p_current_password text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.set_entity_password(p_kind text, p_new_password text, p_current_password text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.set_entity_password(p_kind text, p_new_password text, p_current_password text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 218/248
REVOKE ALL ON FUNCTION public.set_entity_recovery(p_kind text, p_question_id text, p_question_text text, p_answer text, p_current_password text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.set_entity_recovery(p_kind text, p_question_id text, p_question_text text, p_answer text, p_current_password text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.set_entity_recovery(p_kind text, p_question_id text, p_question_text text, p_answer text, p_current_password text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 219/248
REVOKE ALL ON FUNCTION public.set_switch_pin(p_new_pin text, p_current_pin text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.set_switch_pin(p_new_pin text, p_current_pin text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.set_switch_pin(p_new_pin text, p_current_pin text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 220/248
REVOKE ALL ON FUNCTION public.setup_entity_password_with_recovery(p_kind text, p_new_password text, p_question_id text, p_question_text text, p_answer text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.setup_entity_password_with_recovery(p_kind text, p_new_password text, p_question_id text, p_question_text text, p_answer text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.setup_entity_password_with_recovery(p_kind text, p_new_password text, p_question_id text, p_question_text text, p_answer text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 221/248
REVOKE ALL ON FUNCTION public.start_live_share(p_lat double precision, p_lng double precision) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.start_live_share(p_lat double precision, p_lng double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.start_live_share(p_lat double precision, p_lng double precision) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 222/248
REVOKE ALL ON FUNCTION public.stop_live_share() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.stop_live_share() TO authenticated;
GRANT EXECUTE ON FUNCTION public.stop_live_share() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 223/248
REVOKE ALL ON FUNCTION public.stories_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.stories_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer) TO anon;
GRANT EXECUTE ON FUNCTION public.stories_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.stories_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer) TO service_role;
GRANT EXECUTE ON FUNCTION public.stories_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer) TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 224/248
REVOKE ALL ON FUNCTION public.suggest_business_login(p_business_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.suggest_business_login(p_business_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.suggest_business_login(p_business_id text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 225/248
REVOKE ALL ON FUNCTION public.sweep_my_appointments() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.sweep_my_appointments() TO authenticated;
GRANT EXECUTE ON FUNCTION public.sweep_my_appointments() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 226/248
REVOKE ALL ON FUNCTION public.sweep_stale_appointment_holds() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.sweep_stale_appointment_holds() TO authenticated;
GRANT EXECUTE ON FUNCTION public.sweep_stale_appointment_holds() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 227/248
REVOKE ALL ON FUNCTION public.sync_allow_comments_from_policy() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.sync_allow_comments_from_policy() TO anon;
GRANT EXECUTE ON FUNCTION public.sync_allow_comments_from_policy() TO authenticated;
GRANT EXECUTE ON FUNCTION public.sync_allow_comments_from_policy() TO service_role;
GRANT EXECUTE ON FUNCTION public.sync_allow_comments_from_policy() TO PUBLIC;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 228/248
REVOKE ALL ON FUNCTION public.sync_community_post_geom() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.sync_community_post_geom() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 229/248
REVOKE ALL ON FUNCTION public.sync_geom() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.sync_geom() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 230/248
REVOKE ALL ON FUNCTION public.sync_is_verified() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.sync_is_verified() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 231/248
REVOKE ALL ON FUNCTION public.sync_me_too_count() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.sync_me_too_count() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 232/248
REVOKE ALL ON FUNCTION public.sync_post_comments_count() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.sync_post_comments_count() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 233/248
REVOKE ALL ON FUNCTION public.sync_post_likes_count() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.sync_post_likes_count() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 234/248
REVOKE ALL ON FUNCTION public.sync_request_me_too() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.sync_request_me_too() TO authenticated;
GRANT EXECUTE ON FUNCTION public.sync_request_me_too() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 235/248
REVOKE ALL ON FUNCTION public.sync_request_proposal_count() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.sync_request_proposal_count() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 236/248
REVOKE ALL ON FUNCTION public.sync_story_geom() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.sync_story_geom() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 237/248
REVOKE ALL ON FUNCTION public.tg_complete_appointment_on_delivery() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.tg_complete_appointment_on_delivery() TO authenticated;
GRANT EXECUTE ON FUNCTION public.tg_complete_appointment_on_delivery() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 238/248
REVOKE ALL ON FUNCTION public.tg_delivery_batch_in_progress() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.tg_delivery_batch_in_progress() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 239/248
REVOKE ALL ON FUNCTION public.trg_bulk_deal_pledge_check_target() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.trg_bulk_deal_pledge_check_target() TO authenticated;
GRANT EXECUTE ON FUNCTION public.trg_bulk_deal_pledge_check_target() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 240/248
REVOKE ALL ON FUNCTION public.update_delivery_batch_position(p_batch_id text, p_lat double precision, p_lng double precision, p_accuracy double precision, p_heading double precision) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.update_delivery_batch_position(p_batch_id text, p_lat double precision, p_lng double precision, p_accuracy double precision, p_heading double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.update_delivery_batch_position(p_batch_id text, p_lat double precision, p_lng double precision, p_accuracy double precision, p_heading double precision) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 241/248
REVOKE ALL ON FUNCTION public.update_delivery_position(p_delivery_id text, p_lat double precision, p_lng double precision, p_accuracy double precision, p_heading double precision) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.update_delivery_position(p_delivery_id text, p_lat double precision, p_lng double precision, p_accuracy double precision, p_heading double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.update_delivery_position(p_delivery_id text, p_lat double precision, p_lng double precision, p_accuracy double precision, p_heading double precision) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 242/248
REVOKE ALL ON FUNCTION public.update_live_share(p_lat double precision, p_lng double precision, p_accuracy double precision, p_heading double precision) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.update_live_share(p_lat double precision, p_lng double precision, p_accuracy double precision, p_heading double precision) TO authenticated;
GRANT EXECUTE ON FUNCTION public.update_live_share(p_lat double precision, p_lng double precision, p_accuracy double precision, p_heading double precision) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 243/248
REVOKE ALL ON FUNCTION public.update_rating_avg() FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.update_rating_avg() TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 244/248
REVOKE ALL ON FUNCTION public.update_team_member_scopes(p_session_id uuid, p_scopes text[]) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.update_team_member_scopes(p_session_id uuid, p_scopes text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.update_team_member_scopes(p_session_id uuid, p_scopes text[]) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 245/248
REVOKE ALL ON FUNCTION public.verify_business_password(p_business_id text, p_password text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.verify_business_password(p_business_id text, p_password text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.verify_business_password(p_business_id text, p_password text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 246/248
REVOKE ALL ON FUNCTION public.verify_provider_password(p_provider_id text, p_password text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.verify_provider_password(p_provider_id text, p_password text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.verify_provider_password(p_provider_id text, p_password text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 247/248
REVOKE ALL ON FUNCTION public.verify_switch_pin(p_pin text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.verify_switch_pin(p_pin text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.verify_switch_pin(p_pin text) TO service_role;

-- @@item Function EXECUTE grants (exact: revoke defaults, then grant) 248/248
REVOKE ALL ON FUNCTION public.withdraw_proposal(p_proposal_id text) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.withdraw_proposal(p_proposal_id text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.withdraw_proposal(p_proposal_id text) TO service_role;
