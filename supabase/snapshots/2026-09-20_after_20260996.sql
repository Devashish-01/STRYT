...
-- taken:    2026-09-20T09:19:23.714Z
...
CREATE TABLE public.admin_actions (
  id text DEFAULT ('aa_'::text || replace((gen_random_uuid())::text, '-'::text, ''::text)) NOT NULL,
...
CREATE TABLE public.entity_password_attempts (
  ...
  locked_until timestamp with time zone,
  last_attempt_at timestamp with time zone DEFAULT now() NOT NULL,
  attempted_by text DEFAULT ''::text NOT NULL,
...
CREATE TABLE public.events (
  id text DEFAULT ('ev_'::text || replace((gen_random_uuid())::text, '-'::text, ''::text)) NOT NULL,
  user_id text DEFAULT (auth.uid())::text,
  session_id text,
  name text NOT NULL,
...
CREATE TABLE public.moderation_settings (
  ...
  report_hide_threshold integer DEFAULT 5 NOT NULL,
...
ALTER TABLE ONLY admin_actions ADD CONSTRAINT admin_actions_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY bulk_deals ADD CONSTRAINT bulk_deals_hidden_reason_check CHECK (((hidden_reason IS NULL) OR (hidden_reason = ANY (ARRAY['REPORTS'::text, 'AUTO_CHECK'::text, 'REMOVED'::text]))));
...
ALTER TABLE ONLY community_posts ADD CONSTRAINT community_posts_hidden_reason_check CHECK (((hidden_reason IS NULL) OR (hidden_reason = ANY (ARRAY['REPORTS'::text, 'AUTO_CHECK'::text, 'REMOVED'::text]))));
...
ALTER TABLE ONLY customer_onboarding_state ADD CONSTRAINT customer_onboarding_state_step_check CHECK (((step >= 0) AND (step <= 4)));

ALTER TABLE ONLY customer_onboarding_state ADD CONSTRAINT customer_onboarding_state_user_id_fkey FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;

ALTER TABLE ONLY customer_onboarding_state ADD CONSTRAINT customer_onboarding_state_pkey PRIMARY KEY (user_id);
...
ALTER TABLE ONLY entity_password_attempts ADD CONSTRAINT entity_password_attempts_pkey PRIMARY KEY (owner_user_id, kind, attempted_by);
...
ALTER TABLE ONLY events ADD CONSTRAINT events_name_known CHECK ((name = ANY (ARRAY['signup_completed'::text, 'request_created'::text, 'request_viewed'::text, 'proposal_created'::text, 'proposal_accepted'::text, 'booking_created'::text, 'deal_completed'::text, 'dispute_opened'::text, 'review_left'::text, 'share_link_opened'::text])));

ALTER TABLE ONLY events ADD CONSTRAINT events_props_small CHECK ((pg_column_size(props) <= 2048));
...
ALTER TABLE ONLY moderation_settings ADD CONSTRAINT moderation_settings_id_check CHECK (id);

ALTER TABLE ONLY moderation_settings ADD CONSTRAINT moderation_settings_report_hide_threshold_check CHECK (((report_hide_threshold >= 2) AND (report_hide_threshold <= 100)));

ALTER TABLE ONLY moderation_settings ADD CONSTRAINT moderation_settings_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY post_comments ADD CONSTRAINT post_comments_hidden_reason_check CHECK (((hidden_reason IS NULL) OR (hidden_reason = ANY (ARRAY['REPORTS'::text, 'AUTO_CHECK'::text, 'REMOVED'::text]))));
...
ALTER TABLE ONLY ratings ADD CONSTRAINT ratings_hidden_reason_check CHECK (((hidden_reason IS NULL) OR (hidden_reason = ANY (ARRAY['REPORTS'::text, 'AUTO_CHECK'::text, 'REMOVED'::text]))));
...
ALTER TABLE ONLY requests ADD CONSTRAINT requests_hidden_reason_check CHECK (((hidden_reason IS NULL) OR (hidden_reason = ANY (ARRAY['REPORTS'::text, 'AUTO_CHECK'::text, 'REMOVED'::text]))));
...
ALTER TABLE ONLY stories ADD CONSTRAINT stories_hidden_reason_check CHECK (((hidden_reason IS NULL) OR (hidden_reason = ANY (ARRAY['REPORTS'::text, 'AUTO_CHECK'::text, 'REMOVED'::text]))));
...
CREATE OR REPLACE FUNCTION public.category_counts_nearby(in_lng double precision DEFAULT NULL::double precision, in_lat double precision DEFAULT NULL::double precision, in_radius_km double precision DEFAULT NULL::double precision)
...
CREATE OR REPLACE FUNCTION public.customer_onboarding(p_action text DEFAULT 'read'::text, p_payload jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
begin
  ...
  select u.id, 'NEARBY_REQUEST',
   ...
   where u.id <> new.requester_user_id
     and u.lat is not null and u.lng is not null
     -- Shops and providers already told about this request by the targeted inserts above don't get it twice.
     and not exists (
       select 1 from public.notifications n
        where n.user_id = u.id and n.type = 'NEARBY_REQUEST' and n.metadata->>'requestId' = new.id
     )
...
CREATE OR REPLACE FUNCTION public.record_login_acceptance(p_version text, p_attempt_id uuid, p_user_agent text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.search_businesses_nearby(in_q text, in_lng double precision DEFAULT NULL::double precision, in_lat double precision DEFAULT NULL::double precision, in_radius_km double precision DEFAULT 25, in_limit integer DEFAULT 20, in_offset integer DEFAULT 0)
 RETURNS SETOF businesses
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.search_providers_nearby(in_q text, in_lng double precision DEFAULT NULL::double precision, in_lat double precision DEFAULT NULL::double precision, in_radius_km double precision DEFAULT 25, in_limit integer DEFAULT 20, in_offset integer DEFAULT 0)
 RETURNS SETOF providers
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
...
-- ════════════════════════════════════════════════════════════
-- Function EXECUTE grants (effective) (265)
-- ════════════════════════════════════════════════════════════

GRANT EXECUTE ON FUNCTION public._bulk_deal_close_internal(p_deal_id text, p_trigger text, p_outcome text) TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public._enforce_business_owner_limit() TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.admin_moderation_remove(p_target_type text, p_target_id text) TO authenticated, postgres, service_role;

GRANT EXECUTE ON FUNCTION public.admin_moderation_restore(p_target_type text, p_target_id text) TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.broadcast_new_listing(p_type text, p_id text) TO authenticated, postgres, service_role;

GRANT EXECUTE ON FUNCTION public.broadcast_offer_to_nearby(p_offer_id text, p_radius_km double precision) TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.catalog_item_adjust_quantity(p_id text, p_delta integer) TO authenticated, postgres, service_role;

GRANT EXECUTE ON FUNCTION public.category_counts_nearby(in_lng double precision, in_lat double precision, in_radius_km double precision) TO anon, authenticated, postgres, service_role, PUBLIC;

GRANT EXECUTE ON FUNCTION public.check_bulk_deal_target_and_close(p_deal_id text) TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.check_self_vouch() TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.customer_onboarding(p_action text, p_payload jsonb) TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.emergency_contact_add_by_identifier(p_identifier text) TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.enforce_emergency_contact_cap() TO anon, authenticated, postgres, service_role, PUBLIC;

GRANT EXECUTE ON FUNCTION public.enforce_manual_verification_decision() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.enforce_places_status_freeze() TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.grant_team_access(p_business_id text, p_identifier text, p_scopes text[]) TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.guard_moderation_columns() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.guard_moderation_columns_on_insert() TO postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.hide_after_reports() TO postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.log_admin_action() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.marketplace_health(p_days integer) TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.notify_admins_business_pending() TO postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.notify_on_chat_message() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.notify_on_comment_mention() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.notify_on_comment_reaction() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.notify_on_nearby_alert() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.notify_on_post_comment() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.notify_on_post_like() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.notify_on_post_recommendation() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.notify_on_post_resolved() TO postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.notify_on_qna_answered() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.notify_on_qna_asked() TO authenticated, postgres, service_role;

GRANT EXECUTE ON FUNCTION public.notify_on_queue_called() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.notify_on_queue_payment_status() TO authenticated, postgres, service_role;

GRANT EXECUTE ON FUNCTION public.notify_on_rating() TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.provider_leads(p_provider_id text) TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.recompute_rating_aggregates() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.record_login_acceptance(p_version text, p_attempt_id uuid, p_user_agent text) TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.request_content_check() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.request_location_share(p_owner text) TO authenticated, postgres, service_role;

GRANT EXECUTE ON FUNCTION public.request_payment_nudge(p_kind text, p_id text) TO authenticated, postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.reserve_catalog_item(p_item_id text) TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.reserve_catalog_items(p_items jsonb) TO postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.search_businesses_nearby(in_q text, in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer, in_offset integer) TO anon, authenticated, postgres, service_role, PUBLIC;

GRANT EXECUTE ON FUNCTION public.search_providers_nearby(in_q text, in_lng double precision, in_lat double precision, in_radius_km double precision, in_limit integer, in_offset integer) TO anon, authenticated, postgres, service_role, PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.sync_post_likes_count() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.sync_request_me_too() TO authenticated, postgres, service_role;

GRANT EXECUTE ON FUNCTION public.sync_request_proposal_count() TO postgres, service_role;
...
GRANT EXECUTE ON FUNCTION public.tg_delivery_batch_in_progress() TO postgres, service_role;

GRANT EXECUTE ON FUNCTION public.trg_bulk_deal_pledge_check_target() TO authenticated, postgres, service_role;
...
CREATE TRIGGER trg_admin_audit_agreements AFTER UPDATE OF status ON agreements FOR EACH ROW WHEN (old.status IS DISTINCT FROM new.status) EXECUTE FUNCTION log_admin_action('status');
...
CREATE TRIGGER trg_admin_audit_bug_reports AFTER UPDATE OF status ON bug_reports FOR EACH ROW WHEN (old.status IS DISTINCT FROM new.status) EXECUTE FUNCTION log_admin_action('status');
...
CREATE TRIGGER trg_content_check_bulk_deals AFTER INSERT OR UPDATE OF title, description ON bulk_deals FOR EACH ROW EXECUTE FUNCTION request_content_check('BULK_DEAL');

CREATE TRIGGER trg_guard_moderation_bulk_deals BEFORE UPDATE OF hidden_at, hidden_reason ON bulk_deals FOR EACH ROW EXECUTE FUNCTION guard_moderation_columns();

CREATE TRIGGER trg_guard_moderation_insert_bulk_deals BEFORE INSERT ON bulk_deals FOR EACH ROW EXECUTE FUNCTION guard_moderation_columns_on_insert();
...
CREATE TRIGGER trg_admin_audit_businesses AFTER UPDATE OF status, location_review_status ON businesses FOR EACH ROW WHEN (old.status IS DISTINCT FROM new.status OR old.location_review_status IS DISTINCT FROM new.location_review_status) EXECUTE FUNCTION log_admin_action('status', 'location_review_status', 'lat', 'lng');

CREATE TRIGGER trg_content_check_businesses AFTER INSERT OR UPDATE OF name, description ON businesses FOR EACH ROW EXECUTE FUNCTION request_content_check('BUSINESS');
...
CREATE TRIGGER trg_admin_audit_community_posts AFTER DELETE ON community_posts FOR EACH ROW EXECUTE FUNCTION log_admin_action();

CREATE TRIGGER trg_content_check_community_posts AFTER INSERT OR UPDATE OF title, body ON community_posts FOR EACH ROW EXECUTE FUNCTION request_content_check('POST');

CREATE TRIGGER trg_guard_moderation_community_posts BEFORE UPDATE OF hidden_at, hidden_reason ON community_posts FOR EACH ROW EXECUTE FUNCTION guard_moderation_columns();

CREATE TRIGGER trg_guard_moderation_insert_community_posts BEFORE INSERT ON community_posts FOR EACH ROW EXECUTE FUNCTION guard_moderation_columns_on_insert();
...
CREATE TRIGGER trg_emergency_contact_cap BEFORE INSERT ON emergency_contacts FOR EACH ROW EXECUTE FUNCTION enforce_emergency_contact_cap();
...
CREATE TRIGGER trg_admin_audit_places AFTER UPDATE OF status ON places FOR EACH ROW WHEN (old.status IS DISTINCT FROM new.status) EXECUTE FUNCTION log_admin_action('status');
...
CREATE TRIGGER trg_content_check_post_comments AFTER INSERT OR UPDATE OF body ON post_comments FOR EACH ROW EXECUTE FUNCTION request_content_check('COMMENT');

CREATE TRIGGER trg_guard_moderation_insert_post_comments BEFORE INSERT ON post_comments FOR EACH ROW EXECUTE FUNCTION guard_moderation_columns_on_insert();

CREATE TRIGGER trg_guard_moderation_post_comments BEFORE UPDATE OF hidden_at, hidden_reason ON post_comments FOR EACH ROW EXECUTE FUNCTION guard_moderation_columns();
...
CREATE TRIGGER trg_admin_audit_providers AFTER UPDATE OF status ON providers FOR EACH ROW WHEN (old.status IS DISTINCT FROM new.status) EXECUTE FUNCTION log_admin_action('status');

CREATE TRIGGER trg_content_check_providers AFTER INSERT OR UPDATE OF display_name, bio ON providers FOR EACH ROW EXECUTE FUNCTION request_content_check('PROVIDER');
...
CREATE TRIGGER trg_content_check_ratings AFTER INSERT OR UPDATE OF comment ON ratings FOR EACH ROW WHEN (COALESCE(new.comment, ''::text) <> ''::text) EXECUTE FUNCTION request_content_check('RATING');

CREATE TRIGGER trg_guard_moderation_insert_ratings BEFORE INSERT ON ratings FOR EACH ROW EXECUTE FUNCTION guard_moderation_columns_on_insert();

CREATE TRIGGER trg_guard_moderation_ratings BEFORE UPDATE OF hidden_at, hidden_reason ON ratings FOR EACH ROW EXECUTE FUNCTION guard_moderation_columns();
...
CREATE TRIGGER trg_admin_audit_reports AFTER UPDATE OF status ON reports FOR EACH ROW WHEN (old.status IS DISTINCT FROM new.status) EXECUTE FUNCTION log_admin_action('status');

CREATE TRIGGER trg_hide_after_reports AFTER INSERT ON reports FOR EACH ROW EXECUTE FUNCTION hide_after_reports();
...
CREATE TRIGGER trg_admin_audit_requests AFTER UPDATE OF status ON requests FOR EACH ROW WHEN (old.status IS DISTINCT FROM new.status) EXECUTE FUNCTION log_admin_action('status');

CREATE TRIGGER trg_content_check_requests AFTER INSERT OR UPDATE OF title, description ON requests FOR EACH ROW EXECUTE FUNCTION request_content_check('REQUEST');

CREATE TRIGGER trg_guard_moderation_insert_requests BEFORE INSERT ON requests FOR EACH ROW EXECUTE FUNCTION guard_moderation_columns_on_insert();

CREATE TRIGGER trg_guard_moderation_requests BEFORE UPDATE OF hidden_at, hidden_reason ON requests FOR EACH ROW EXECUTE FUNCTION guard_moderation_columns();

CREATE TRIGGER trg_notify_request AFTER INSERT ON requests FOR EACH ROW EXECUTE FUNCTION notify_on_request();

CREATE TRIGGER trg_content_check_stories AFTER INSERT OR UPDATE OF caption ON stories FOR EACH ROW WHEN (COALESCE(new.caption, ''::text) <> ''::text) EXECUTE FUNCTION request_content_check('STORY');

CREATE TRIGGER trg_guard_moderation_insert_stories BEFORE INSERT ON stories FOR EACH ROW EXECUTE FUNCTION guard_moderation_columns_on_insert();

CREATE TRIGGER trg_guard_moderation_stories BEFORE UPDATE OF hidden_at, hidden_reason ON stories FOR EACH ROW EXECUTE FUNCTION guard_moderation_columns();
...
-- ════════════════════════════════════════════════════════════
-- Indexes (not backing a constraint) (189)
-- ════════════════════════════════════════════════════════════
...
CREATE INDEX bulk_deal_tokens_holder_idx ON public.bulk_deal_tokens USING btree (holder_user_id);

CREATE INDEX bulk_deal_tokens_issuer_idx ON public.bulk_deal_tokens USING btree (issuer_user_id);
...
CREATE INDEX bulk_deals_owner_idx ON public.bulk_deals USING btree (owner_user_id);
...
CREATE INDEX entity_password_attempts_window_idx ON public.entity_password_attempts USING btree (owner_user_id, kind, last_attempt_at DESC);

CREATE INDEX events_created_idx ON public.events USING btree (created_at DESC);

CREATE INDEX events_name_created_idx ON public.events USING btree (name, created_at DESC);

CREATE INDEX events_user_created_idx ON public.events USING btree (user_id, created_at DESC);
...
CREATE INDEX group_buy_tokens_issuer_idx ON public.group_buy_tokens USING btree (issuer_user_id);
...
CREATE UNIQUE INDEX reports_one_open_per_reporter ON public.reports USING btree (reporter_user_id, target_type, target_id) WHERE ((reporter_user_id IS NOT NULL) AND (status = ANY (ARRAY['OPEN'::text, 'REVIEWING'::text])));
...
CREATE UNIQUE INDEX terms_acceptances_login_attempt_unique ON public.terms_acceptances USING btree (user_id, login_attempt_id) WHERE (login_attempt_id IS NOT NULL);
...
CREATE POLICY read_bulk_deal_pledges ON public.bulk_deal_pledges AS PERMISSIVE FOR SELECT TO authenticated
  USING (((user_id = (( SELECT auth.uid() AS uid))::text) OR (EXISTS ( SELECT 1
   FROM bulk_deals d
  WHERE ((d.id = bulk_deal_pledges.deal_id) AND has_business_access(d.business_id, (( SELECT auth.uid() AS uid))::text)))) OR is_admin((( SELECT auth.uid() AS uid))::text)));

CREATE POLICY read_bulk_deal_tokens ON public.bulk_deal_tokens AS PERMISSIVE FOR SELECT TO authenticated
  USING (((holder_user_id = (( SELECT auth.uid() AS uid))::text) OR (issuer_user_id = (( SELECT auth.uid() AS uid))::text) OR ((business_id IS NOT NULL) AND has_business_access(business_id, (( SELECT auth.uid() AS uid))::text)) OR is_admin((( SELECT auth.uid() AS uid))::text)));

CREATE POLICY read_bulk_deals ON public.bulk_deals AS PERMISSIVE FOR SELECT TO PUBLIC
  USING ((((status = 'ACTIVE'::entity_status) AND (hidden_at IS NULL)) OR (owner_user_id = (( SELECT auth.uid() AS uid))::text) OR is_admin()));
...
CREATE POLICY "Allow insert access to community_posts for authenticated" ON public.community_posts AS PERMISSIVE FOR INSERT TO PUBLIC
  WITH CHECK (((( SELECT auth.role() AS role) = 'authenticated'::text) AND (author_user_id = (( SELECT auth.uid() AS uid))::text)));

CREATE POLICY "Allow read access to community_posts for all" ON public.community_posts AS PERMISSIVE FOR SELECT TO PUBLIC
  USING (((NOT is_blocked_between(author_user_id)) AND ((hidden_at IS NULL) OR (author_user_id = (( SELECT auth.uid() AS uid))::text) OR ( SELECT is_admin() AS is_admin))));
...
CREATE POLICY select_custom_payments ON public.custom_payments AS PERMISSIVE FOR SELECT TO authenticated
  USING ((((( SELECT auth.uid() AS uid))::text = payer_user_id) OR ((( SELECT auth.uid() AS uid))::text = target_owner_user_id) OR ((target_type = 'BUSINESS'::text) AND has_business_scope(target_id, (( SELECT auth.uid() AS uid))::text, 'appointments'::text)) OR is_admin()));

CREATE POLICY onboarding_read_own ON public.customer_onboarding_state AS PERMISSIVE FOR SELECT TO authenticated
  USING ((user_id = (( SELECT auth.uid() AS uid))::text));
...
CREATE POLICY events_admin_read ON public.events AS PERMISSIVE FOR SELECT TO authenticated
  USING ((EXISTS ( SELECT 1
   FROM users a
  WHERE ((a.id = (( SELECT auth.uid() AS uid))::text) AND ((a.roles @> ARRAY['admin'::text]) OR (a.roles @> ARRAY['super_admin'::text]))))));

CREATE POLICY events_insert_guest ON public.events AS PERMISSIVE FOR INSERT TO anon
  WITH CHECK ((user_id IS NULL));

CREATE POLICY events_insert_own ON public.events AS PERMISSIVE FOR INSERT TO authenticated
  WITH CHECK ((user_id = (( SELECT auth.uid() AS uid))::text));
...
CREATE POLICY read_group_buy_tokens ON public.group_buy_tokens AS PERMISSIVE FOR SELECT TO PUBLIC
  USING (((holder_user_id = (( SELECT auth.uid() AS uid))::text) OR (issuer_user_id = (( SELECT auth.uid() AS uid))::text) OR is_admin()));
...
CREATE POLICY insert_own_messages ON public.messages AS PERMISSIVE FOR INSERT TO authenticated
  WITH CHECK (((sender_id = (( SELECT auth.uid() AS uid))::text) AND (EXISTS ( SELECT 1
   FROM conversations c
  WHERE ((c.id = messages.conversation_id) AND ((c.participant_a = (( SELECT auth.uid() AS uid))::text) OR (c.participant_b = (( SELECT auth.uid() AS uid))::text)) AND (NOT is_blocked_between(
        CASE
            WHEN (c.participant_a = (( SELECT auth.uid() AS uid))::text) THEN c.participant_b
...
CREATE POLICY moderation_settings_admin_read ON public.moderation_settings AS PERMISSIVE FOR SELECT TO authenticated
  USING (( SELECT is_admin() AS is_admin));

CREATE POLICY moderation_settings_admin_update ON public.moderation_settings AS PERMISSIVE FOR UPDATE TO authenticated
  USING (( SELECT is_admin() AS is_admin))
...
CREATE POLICY insert_notifications ON public.notifications AS PERMISSIVE FOR INSERT TO authenticated
  WITH CHECK (((user_id = (( SELECT auth.uid() AS uid))::text) OR ( SELECT is_admin() AS is_admin)));
...
CREATE POLICY insert_places ON public.places AS PERMISSIVE FOR INSERT TO PUBLIC
  WITH CHECK (((submitted_by_user_id = (( SELECT auth.uid() AS uid))::text) AND ((status = 'PENDING'::entity_status) OR is_admin())));

CREATE POLICY select_places ON public.places AS PERMISSIVE FOR SELECT TO PUBLIC
  USING (((status = 'ACTIVE'::entity_status) OR (submitted_by_user_id = (( SELECT auth.uid() AS uid))::text) OR is_admin()));
...
CREATE POLICY update_places_owner ON public.places AS PERMISSIVE FOR UPDATE TO PUBLIC
  USING ((submitted_by_user_id = (( SELECT auth.uid() AS uid))::text))
  WITH CHECK ((submitted_by_user_id = (( SELECT auth.uid() AS uid))::text));
...
CREATE POLICY "Allow read access to post_comments for all" ON public.post_comments AS PERMISSIVE FOR SELECT TO PUBLIC
  USING (((hidden_at IS NULL) OR (author_user_id = (( SELECT auth.uid() AS uid))::text) OR ( SELECT is_admin() AS is_admin)));
...
CREATE POLICY read_ratings ON public.ratings AS PERMISSIVE FOR SELECT TO PUBLIC
  USING (((hidden_at IS NULL) OR (rater_user_id = (( SELECT auth.uid() AS uid))::text) OR ( SELECT is_admin() AS is_admin)));

CREATE POLICY upd_ratings ON public.ratings AS PERMISSIVE FOR UPDATE TO PUBLIC
  USING ((rater_user_id = (( SELECT auth.uid() AS uid))::text))
  WITH CHECK ((rater_user_id = (( SELECT auth.uid() AS uid))::text));
...
CREATE POLICY ins_reports ON public.reports AS PERMISSIVE FOR INSERT TO authenticated
  WITH CHECK ((reporter_user_id = (( SELECT auth.uid() AS uid))::text));
...
CREATE POLICY read_requests ON public.requests AS PERMISSIVE FOR SELECT TO PUBLIC
  USING (((hidden_at IS NULL) OR (requester_user_id = (( SELECT auth.uid() AS uid))::text) OR ( SELECT is_admin() AS is_admin)));
...
CREATE POLICY stories_select ON public.stories AS PERMISSIVE FOR SELECT TO PUBLIC
  USING (((((visibility = 'everyone'::text) AND (NOT COALESCE(((( SELECT auth.uid() AS uid))::text = ANY (hidden_user_ids)), false))) OR ((visibility = 'close_friends'::text) AND COALESCE(((( SELECT auth.uid() AS uid))::text = ANY (allowed_user_ids)), false)) OR ((user_id IS NOT NULL) AND (user_id = (( SELECT auth.uid() AS uid))::text)) OR ((( SELECT auth.uid() AS uid) IS NOT NULL) AND (owner_type = 'business'::text) AND can_manage_business(owner_id))) AND ((hidden_at IS NULL) OR (user_id = (( SELECT auth.uid() AS uid))::text) OR ((owner_type = 'business'::text) AND can_manage_business(owner_id)) OR ((owner_type = 'provider'::text) AND (EXISTS ( SELECT 1
   FROM providers p
  WHERE ((p.id = stories.owner_id) AND (p.user_id = (( SELECT auth.uid() AS uid))::text))))) OR ( SELECT is_admin() AS is_admin))));
...
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON public.admin_actions TO service_role;
...
GRANT DELETE, INSERT, REFERENCES, TRIGGER, TRUNCATE, UPDATE ON public.appointment_deliveries TO anon;

GRANT DELETE, INSERT, REFERENCES, TRIGGER, TRUNCATE, UPDATE ON public.appointment_deliveries TO authenticated;
...
GRANT DELETE, INSERT, REFERENCES, TRIGGER, TRUNCATE, UPDATE ON public.business_login_credentials TO authenticated;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON public.customer_onboarding_state TO service_role;
...
GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON public.moderation_settings TO authenticated;

GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON public.moderation_settings TO service_role;
...
-- ════════════════════════════════════════════════════════════
-- Migration ledger (supabase_migrations.schema_migrations) (183)
-- ════════════════════════════════════════════════════════════
...
-- 20260912193124  20260958_capture_database_only_objects

-- 20260912215358  20260897_daily_limit_advisory_lock

-- 20260912215401  20260935_reschedule_preserve_payment_and_package

-- 20260912215403  20260947_appointment_notifications_v2

-- 20260912215406  20260948_delivery_notifications_v2

-- 20260912215408  20260949_community_notifications_v2

-- 20260912215410  20260950_bulk_deal_notifications_v2

-- 20260912215418  20260951_location_notifications_v2

-- 20260912215421  20260952_custom_payment_notifications_v2

-- 20260912215424  20260953_trust_safety_notifications_v2

-- 20260912215426  20260954_discovery_notifications_v2

-- 20260912215428  20260955_proposal_notifications_v2

-- 20260912215431  20260956_identity_system_notifications_v2

-- 20260912222837  20260959_group_buy_unlock_null_owner_guard

-- 20260912230006  20260960_queue_tokens_stage3_lockdown

-- 20260914200956  20260961_ci_readonly_role

-- 20260914203200  20260962_pin_function_search_paths

-- 20260914204000  20260963_revoke_anon_security_definers

-- 20260914205500  20260964_drop_obsolete_bulk_deal_token_redeem

-- 20260914212500  20260965_harden_secdef_authorization

-- 20260914213500  20260966_column_level_pii_lockdown

-- 20260914221353  20260967_hide_handoff_code_from_rpc_results

-- 20260915163206  20260968_users_sensitive_columns_lockdown

-- 20260915163208  20260969_agreements_no_direct_client_writes

-- 20260915163209  20260970_appointments_customer_updates_via_functions

-- 20260915163211  20260971_stories_ownership_and_visibility

-- 20260915163940  20260972_provider_leads_rpc_and_lead_sender_check

-- 20260918194927  20260973_messages_insert_policy_block_check

-- 20260918194930  20260974_fix_dead_column_references

-- 20260918194932  20260975_request_notifications_match_category_group

-- 20260918194935  20260976_proposal_counter_dead_column

-- 20260918194937  20260977_request_notifications_no_duplicates

-- 20260918194939  20260978_admin_actions_audit_log

-- 20260918194942  20260979_bulk_deal_close_deposit_column

-- 20260918194946  20260980_notifications_insert_authorized

-- 20260918194948  20260981_deletion_request_prev_visibility

-- 20260918194950  20260982_moderation_tells_the_author

-- 20260918194953  20260983_entity_password_lockout_per_attempter

-- 20260918194956  20260984_password_change_notice

-- 20260918194958  20260985_catalog_stock_delta

-- 20260918195000  20260986_search_nearby_and_category_counts

-- 20260918195003  20260987_live_share_ends_with_last_recipient

-- 20260918195005  20260988_emergency_contact_by_identifier

-- 20260918195007  20260989_rls_initplan_and_fk_indexes

-- 20260918195010  20260990_community_moderation

-- 20260918195012  20260991_moderation_text_everywhere

-- 20260920045700  20260992_tracking_tokens_not_world_readable

-- 20260920055035  20260993_customer_onboarding

-- 20260920072620  20260994_users_phone_column_lockdown

-- 20260920091820  20260995_drop_escrow_surface

-- 20260920091824  20260996_product_analytics
